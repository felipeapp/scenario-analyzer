...
public class ComunidadeVirtualDao extends GenericSigaaDAO {
	...
	public List<ComunidadeVirtual> findComunidadesComProjecaoByPessoa(Usuario usuario) throws DAOException {
		Query query = getSession().createQuery(
				" select distinct c.id, c.ativa, c.nome from MembroComunidade mc left join mc.comunidade c " +
	...
	public List<NoticiaComunidade> findNoticiasByComunidade(ComunidadeVirtual comunidade) throws HibernateException, DAOException {
		Query q = getSession().createQuery("select nc from NoticiaComunidade nc where nc.comunidade.id = :idComunidade");
	 ...
	 * @return
	 */
	public List<IndicacaoReferenciaComunidade> findReferenciasTurma(ComunidadeVirtual comunidade) {
		DetachedCriteria c = DetachedCriteria.forClass(IndicacaoReferenciaComunidade.class);
		c.addOrder(asc("descricao")).createCriteria("topico").add(eq("comunidade", comunidade));
		@SuppressWarnings("unchecked")
		List<IndicacaoReferenciaComunidade> lista = getHibernateTemplate().findByCriteria(c);
		return lista;
	}
	 ...
	 * @return
	 */
	public List<ConteudoComunidade> findConteudoComunidade(ComunidadeVirtual comunidade) {
		DetachedCriteria c = DetachedCriteria.forClass(ConteudoComunidade.class);
		c.addOrder(asc("titulo")).createCriteria("topico").add(eq("comunidade", comunidade));
		@SuppressWarnings("unchecked")
		List<ConteudoComunidade> lista = getHibernateTemplate().findByCriteria(c);
		return lista;
	}
	...
	public Set<MembroComunidade> findParticipantes(ComunidadeVirtual comunidade) throws DAOException  {
		...
		String hql = "select " + projecao +
				" from MembroComunidade where comunidade.id = ? " +
	...
	public List <MembroComunidade> findParticipantes (ComunidadeVirtual comunidade, PagingInformation paging, boolean crescente) throws DAOException  {
		...
		try {
			String sql = "select m.id, m.id_pessoa, p.nome, m.permissao, id_comunidade from cv.membro_comunidade m join comum.pessoa p using (id_pessoa) where id_comunidade = :idComunidade order by p.nome " + (crescente ? "desc" : "");
			...
			for (MembroComunidade membro : membros) 
				idsPessoas.add(membro.getPessoa().getId());
