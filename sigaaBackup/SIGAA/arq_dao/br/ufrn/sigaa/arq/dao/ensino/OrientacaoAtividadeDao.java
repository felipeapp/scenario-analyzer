...
/** DAO responsável por consultas específicas à OrientacaoAtividade.
 * @author Édipo Elder F. Melo
 *
 */
public class OrientacaoAtividadeDao extends GenericSigaaDAO {
	
	/** Construtor padrão. */
	public OrientacaoAtividadeDao() {
	...
	public Collection<OrientacaoAtividade> findByOrientadorDiscenteComponente(
			...
			boolean isBuscaRegAtivEspecificas, char nivelEnsino,Integer idCurso)
		...
		if (!isEmpty(nomeDiscente))
			sql.append(" and pd.nome_ascii ilike "+  UFRNUtils.toAsciiUTF8(":nomeDiscente"));
		if (!isEmpty(idCurso))
		...
		if (!isEmpty(nomeDiscente))
			q.setString("nomeDiscente", nomeDiscente + "%");
		if (!isEmpty(idCurso))
