...
/**
 *
 * <p>Contém as regras de negócio para aprovação de uma inscrição em mini atividade de extensão </p>
 *
 * <p> <i> A principal regra é aprovar os participantes apenas se ele foram aprovados para atividade principal. 
 * Se não não pode aprovara participação em uma mini atividade, se não vai participar da principal. 
 * Entra as outras regras de verificação estão o pagamento da taxa de inscrição, e a crição obrigatório 
	...
	public void validate(Movimento mov) throws NegocioException, ArqException {
		...
		try{
			...
			if(! subAtividadeSelecionada.isAtivo()){
				erros.addErro(" A mini atividade de extensão para a qual as inscrições foram realizadas não está mais ativa no sistema. ");
			...
			for (InscricaoAtividadeParticipante inscricaoAtividadeParticipante : inscricoesSelecioandas) {
				...
				if( inscricaoAtividadeParticipante.getInscricaoAtividade().getQuantidadeVagas() 
					...
					erros.addErro(" O período de inscrição "+inscricaoAtividadeParticipante.getInscricaoAtividade().getPeriodoInicioFormatado()
							...
							+" para a mini atividade encontra-se com o número de vagas esgotado.");
				}
				...
				if(! possuiInscricaoAtividadeAprovada){
					erros.addErro(" Não é possível aprovar a inscrição do participante: \""
							+inscricaoAtividadeParticipante.getCadastroParticipante().getNome()
							+"\", pois a inscrição dele para a atividade principal não foi aprovada.");
				}
