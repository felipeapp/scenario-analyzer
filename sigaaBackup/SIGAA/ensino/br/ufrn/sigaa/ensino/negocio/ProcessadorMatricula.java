...
/**
 * Processador para cadastrar matrículas de alunos em turmas.
 *
...
//							dao.updateField(Turma.class, turma.getId(), "totalMatriculados", turma.getTotalMatriculados() + 1);
						...
						if (NivelEnsino.isEnsinoBasico(d.getNivel()) || d.getNivel() == NivelEnsino.LATO) {
							...
							// Carrega a turma, para pegar os números da unidade
			...
			for (DiscenteAdapter d : discentes) {
				...
				for (Turma t : turmas) {
					...
					if (d.getCurso().isPresencial() && tDao.findTotalMatriculados(t.getId()) + discentes.size() > t.getCapacidadeAluno()) {
						sb.append("A turma "+ t.getCodigo() +" da disciplina " + t.getDisciplina().getNome()+ " excedeu a sua capacidade de "+ t.getCapacidadeAluno() +" vaga(s). Não é possível realizar sua matrícula.<br/>");
						erro = true;
					}
					
					if (t.isMatricular()) {

						// caso seja aluno de tecnico, é testado se a turma corresponde à especialidade da turma de entrada
						if (d.getNivel() == NivelEnsino.TECNICO && t.getEspecializacao() != null) {
							...
							if (dt.getTurmaEntradaTecnico() == null) {
								sb.append("O Aluno " + d.getPessoa().getNome() + " está sem turma de entrada definida. Não é possível realizar sua matrícula.<br/>");
								erro = true;
								...
								" pois essa turma é reservada somente para alunos da(s) " +
								...
								if(t.getEspecializacao() != null && t.getEspecializacao().getEspecializacoes() != null && t.getEspecializacao().getEspecializacoes().size() > 0){
									...
									sb.append("O Aluno " + d.getPessoa().getNome() + " não pode cursar a disciplina " + t.getDisciplina().getNome() +
									" pois essa turma é reservada somente para alunos da " +
