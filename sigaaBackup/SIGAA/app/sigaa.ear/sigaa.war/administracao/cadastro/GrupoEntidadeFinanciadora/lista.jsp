...
<f:view>
	...
	<h:outputText value="#{grupoEntidadeFinanciadora.create}" />
	...
	<center>
			...
			<div class="infoAltRem">
				<h:form>
					...
					<h:commandLink action="#{grupoEntidadeFinanciadora.preCadastrar}" value="Cadastrar"/>
	...
	<table class=listagem>
	   <caption class="listagem">Lista dos Grupos de Entidades Financiadoras</caption>
		<thead>
			<td>Nome</td>
			<td></td>
			<td></td>
		</thead>
		<c:forEach items="${grupoEntidadeFinanciadora.allPaginado}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				<td>${item.nome}</td>
				<td width=20>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/alterar.gif" value="Alterar"
						action="#{grupoEntidadeFinanciadora.atualizar}" /></h:form>
				</td>
				<td width=25>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/delete.gif" alt="Remover"
						action="#{grupoEntidadeFinanciadora.remover}" onclick="#{confirmDelete}"/></h:form>
				</td>
			</tr>
		</c:forEach>
	</table>
