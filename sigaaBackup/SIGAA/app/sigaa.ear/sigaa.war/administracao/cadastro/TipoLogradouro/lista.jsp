...
<f:view>
	...
	<h:outputText value="#{tipoLogradouro.create}" />
	...
	<center>
			...
			<div class="infoAltRem">
				<h:form>
					...
					<h:commandLink action="#{tipoLogradouro.preCadastrar}" value="Cadastrar"/>
	...
	<table class=listagem>
	  <caption class="listagem">Lista de Tipos de Logradouro</caption>
		<thead>
			<tr>
				<td>Descrição</td>
				<td></td>
		...
		<c:forEach items="${tipoLogradouro.allPaginado}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				<td>${item.descricao}</td>
				<td width=20>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/alterar.gif" value="Alterar"
						action="#{tipoLogradouro.atualizar}" /></h:form>
				</td>
				<td width=25>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/delete.gif" alt="Remover"
						action="#{tipoLogradouro.remover}" onclick="#{confirmDelete}" /></h:form>
				</td>
			</tr>
		</c:forEach>
	</table>
