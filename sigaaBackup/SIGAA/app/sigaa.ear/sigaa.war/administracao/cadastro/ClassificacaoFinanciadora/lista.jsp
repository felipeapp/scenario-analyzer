...
<f:view>
	...
	<h:outputText value="#{classificacaoFinanciadora.create}" />
	...
	<center>
			...
			<div class="infoAltRem">
				...
				<h:form>
					...
					<h:commandLink action="#{classificacaoFinanciadora.preCadastrar}" value="Cadastrar"/>
	...
	<table class=listagem>
	    <caption class="listagem">Lista de Classificações da Entidade Financiadora</caption>
	    <thead>
			<tr>
				<td>Descrição</td>
				<td colspan="2"></td>
	  ...
	  <c:forEach items="${classificacaoFinanciadora.allPaginado}" var="item" varStatus="status">
		  <tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
			 <td>${item.descricao}</td>
			 <td width=20>
				<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
					image="/img/alterar.gif" value="Alterar"
					action="#{classificacaoFinanciadora.atualizar}" /></h:form>
			</td>
			<td width=25>
				<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
					image="/img/delete.gif" alt="Remover"
					action="#{classificacaoFinanciadora.remover}" onclick="#{confirmDelete}"/></h:form>
			</td>
		   </tr>
		</c:forEach>
	</table>
