...
<f:view>
	...
	<h:outputText value="#{tipoProcedenciaAluno.create}" />
	...
	<center>
			...
			<div class="infoAltRem">
				<h:form>
					...
					<h:commandLink action="#{tipoProcedenciaAluno.preCadastrar}" value="Cadastrar"/>
	...
	<table class=listagem>
		<tr>
			<caption class="listagem">Lista de Tipos de Procedência do Aluno</caption>
			<thead>
				<td>Descrição</td>
				<td></td>
				<td></td>
		...
		<c:forEach items="${tipoProcedenciaAluno.allPaginado}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				<td>${item.descricao}</td>
				<td width=20>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/alterar.gif" value="Alterar"
						action="#{tipoProcedenciaAluno.atualizar}" /></h:form>
				</td>
				<td width=25>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/delete.gif" alt="Remover"
						action="#{tipoProcedenciaAluno.remover}"  onclick="#{confirmDelete}"/></h:form>
				</td>
			</tr>
		</c:forEach>
	</table>
