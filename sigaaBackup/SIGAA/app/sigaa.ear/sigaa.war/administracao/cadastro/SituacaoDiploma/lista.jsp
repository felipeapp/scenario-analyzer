...
<f:view>
	...
	<h:outputText value="#{situacaoDiploma.create}" />
	...
	<center>
			...
			<div class="infoAltRem">
				<h:form>
					...
					<h:commandLink action="#{situacaoDiploma.preCadastrar}" value="Cadastrar"/>
	...
	<table class="listagem">
		<caption class="listagem">Lista de Situação de Diploma</caption>
		<thead>
			<td>Descrição</td>
			<td></td>
			<td></td>
		</thead>
		<c:forEach items="${situacaoDiploma.allPaginado}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				<td>${item.descricao}</td>
				<td width=20>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/alterar.gif" value="Alterar"
						action="#{situacaoDiploma.atualizar}" /></h:form></td>
				<td width=25>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/delete.gif" alt="Remover"
						action="#{situacaoDiploma.remover}" onclick="#{confirmDelete}" /></h:form>
				</td>
			</tr>
		</c:forEach>
	</table>
