...
<f:view>
	...
	<h:outputText value="#{entidadeFinanciadoraEstagio.create}" />
	...
	<center>
			...
			<div class="infoAltRem">
				<h:form>
					...
					<h:commandLink action="#{entidadeFinanciadoraEstagio.preCadastrar}" value="Cadastrar"/>
	...
	<table class=listagem>
		<tr>
			<caption class="listagem">Lista de Entidade Financiadora de Estágio</caption>
			<td>Descrição</td>
			<td>Governamental</td>
			<td></td>
			<td></td>
		</tr>
		<c:forEach items="${entidadeFinanciadoraEstagio.allPaginado}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				<td>${item.descricao}</td>
				<td>${item.governamental}</td>
				<h:form>
					<td width=20><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/alterar.gif" value="Alterar"
						action="#{entidadeFinanciadoraEstagio.atualizar}" /></td>
				</h:form>
				<h:form>
					<td width=25><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/delete.gif" alt="Remover"
						action="#{entidadeFinanciadoraEstagio.preRemover}" /></td>
				</h:form>
			</tr>
		</c:forEach>
	</table>
