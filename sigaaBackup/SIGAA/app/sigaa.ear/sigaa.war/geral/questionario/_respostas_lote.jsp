...
<rich:dataTable var="resposta" value="#{item.respostasModel}" width="100%" id="dataTableQuestionario" rowKeyVar="row"
	rowClasses="linhaPar, linhaImpar" >

	<rich:column>
		<h:outputText value="#{row + 1}. #{resposta.pergunta.pergunta}" styleClass="pergunta" /> 
		<h:outputText value="#{resposta.questionarioRespostas.popularVisualizacaoRespostas}"/>
		...
		<h:panelGroup rendered="#{resposta.pergunta.arquivo}">
			<rich:panel>
				<a href='/sigaa/verProducao?idProducao=${resposta.respostaArquivo}&key=${ sf:generateArquivoKey(resposta.respostaArquivo) }' target='_blank'><img src='/img/download.png'/></a>
			</rich:panel>
		</h:panelGroup>
