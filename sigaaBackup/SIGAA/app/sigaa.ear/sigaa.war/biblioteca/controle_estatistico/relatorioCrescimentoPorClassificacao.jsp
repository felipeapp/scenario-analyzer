...
<f:view>
		...
		<table class="tabelaRelatorioBorda" style="width:100%;margin-top: 40px;">
			...
			<tr>
				...
				<c:forEach var="totalPorClasse" items="#{_abstractRelatorioBiblioteca.quantidadeAnteriorTitulo}">
					<th style="text-align: right; width: 800px;"> ${totalPorClasse.key}</th>
				</c:forEach>
			...
			<tr>
				<td></td>
				<c:forEach var="totalPorClasse" items="#{_abstractRelatorioBiblioteca.quantidadeAnteriorTitulo}">
					<td style="width:  800px; text-align: right;"> ${totalPorClasse.value}</td>
			...
			<c:forEach var="dadosPorMes" items="#{dadosDoCrescimentoTitulo.dadosInternos}" >
				...
				<tr>
					<c:forEach var="dadosPorAgrupamento" items="#{dadosPorMes.dadosInternos}" >
						<td style="font-weight: bold; width: 80px; text-align: center;">${dadosPorAgrupamento.descricaoAgrupamento}</td>
						<td>
							<table style="width: 100%; border: 0px;">
								<tr>
									<!--  Imprime os cabeçalhos -->
									<c:forEach var="dadosInternos" items="#{dadosPorAgrupamento.dadosInternos}" >
										<th style="width: 800px;text-align: right;">${dadosInternos.classificacao}</th>
									</c:forEach>
	...
	<table class="tabelaRelatorioBorda" style="width:100%; margin-top: 40px;">
		...
		<c:set var="totalGeralTitulosAgrupado" value="0" scope="request" />
		
		
		<c:forEach var="dadosDoCrescimentoTitulosAgrupado" items="#{_abstractRelatorioBiblioteca.resultadoCrescimentoTituloAgrupado}" >
			<tr>
				<td colspan="23" style="background:#6A6A6A;font-weight:bold;color: white;"> 	${dadosDoCrescimentoTitulosAgrupado.ano} </td>
			</tr>
			
			
			<c:forEach var="dadosPorMes" items="#{dadosDoCrescimentoTitulosAgrupado.dadosInternos}" >
				<tr>
				...
				<tr>
					...
					<c:forEach var="classe" items="#{classes}">                         
						<th style="width: 800px; text-align:right;">${ classe }</th>
					</c:forEach>
				...
				<tr>
					...
					<c:forEach var="totalPorClasse" items="#{dadosPorMes.totaisPorClasificacao}">
						<td style="width: 800px; margin: 0px; padding: 0px; font-weight: bold; text-align: right;"> ${totalPorClasse.value}</td>
					</c:forEach>	
			...
			<td colspan="23" style="font-weight:bold;"> Total por Ano: ${dadosDoCrescimentoTitulosAgrupado.totalPorAno}</td>
			
			<%-- Soma o total de cada ano --%>
			<c:if test="${! _abstractRelatorioBiblioteca.dadoBooleano}" >   
				<c:set var="totalGeralTitulosAgrupado" value="${totalGeralTitulosAgrupado + dadosDoCrescimentoTitulosAgrupado.totalPorAno}" scope="request" />
			</c:if>
			<c:if test="${_abstractRelatorioBiblioteca.dadoBooleano}" > 
				<c:set var="totalGeralTitulosAgrupado" value="${dadosDoCrescimentoTitulosAgrupado.totalPorAno}" scope="request" />  
			</c:if>
		</c:forEach>
	...
	<table class="tabelaRelatorioBorda" style="width:100%; margin-top: 30px;">
		<tr>
			<td colspan="23" style="font-weight: bold; text-align: center; font-size: 14px;"> Total Geral: ${totalGeralTitulosAgrupado} </td>
		</tr>
	</table>
		...
		<c:forEach var="dadosDoCrescimentoMateriais" items="#{_abstractRelatorioBiblioteca.resultadoCrescimentoMaterial}" >
			...
			<c:forEach var="dadosPorMes" items="#{dadosDoCrescimentoMateriais.dadosInternos}" >
				...
				<tr>
					...
					<td style="font-weight: bold; text-align: right; width: 800px;">  ${dadosPorMes.totalPorMes} </td>
				</tr>	
	...
	<div style="text-align: center; margin: 10px;">
		...
		<cewolf:chart id="grafico" type="stackedverticalbar3d"
				xaxislabel="Classificação" yaxislabel="${ _abstractRelatorioBiblioteca.ctgMaterial == _abstractRelatorioBiblioteca.ctgMatExemplares ? 'Exemplares' : 'Fascículos' }" showlegend="true">
			<cewolf:data>
