...
<f:view>
	...
	<a4j:keepAlive beanName="registraMateriaisInventarioMBean" />
	
	<h:form id="formRetistraConsultaDiariaisMateriasLeitor">
	
		<a4j:region>
	
		<h2><ufrn:subSistema /> &gt; Registrar Materiais no Inventário do Acervo</h2>
		...
		<table class="formulario" width="85%">
			...
			<caption>Registrar Materiais</caption>
				
			<tr>
				<td>
					<table class="subformulario" width="100%">
						
						<caption>Códigos de Barras dos Materiais a Registrar</caption>
						
						<thead>
							<tr>
								<th style="text-align:center;">
									Código de Barras: <h:inputText id="campoCB" 
										onkeypress="return executaClickBotao(event, 'formRetistraConsultaDiariaisMateriasLeitor:botaoAdicionar' )" value="#{registraMateriaisInventarioMBean.codigoBarras}" />
										
										<a4j:commandButton style="margin-left:10px;" id="botaoAdicionar" title="Adicionar Código de Barras"
												actionListener="#{registraMateriaisInventarioMBean.adicionarMaterial}"
						...
						<tr>
							<td>
								<c:if test="${empty registraMateriaisInventarioMBean.codigosBarras}">
									<p style="color:#FF0000;text-align:center;">Não há códigos de barras adicionados para o registro.</p>
								</c:if>
								
								<p style="line-height:25px;">
									<c:forEach var="c" items="#{registraMateriaisInventarioMBean.codigosBarras}">
										${c}&nbsp;&nbsp;&nbsp;&nbsp;
									</c:forEach>
								</p>
							</td>
			...
			<tr>
				...
				<td>
					<table class="subFormulario" style="width:100%;">
						...
						<caption>Inventário de Registro</caption>
								
						<tr>
							<th class="obrigatorio">Biblioteca:</th>
							<td>
								<h:selectOneMenu id="comboboxBibliotecas" value="#{registraMateriaisInventarioMBean.biblioteca}" valueChangeListener="#{registraMateriaisInventarioMBean.buscarInventarios}" onchange="submit();">
									<f:selectItem itemLabel="-- SELECIONE --" itemValue="-1" />
									<f:selectItems value="#{registraMateriaisInventarioMBean.bibliotecasInternas}"/>
								</h:selectOneMenu>
							</td>
						</tr>
								
						<%-- <tr style="${fn:length(registraMateriaisInventarioMBean.inventarios) > 0 ? 'display: auto' : 'display: none'}"> --%>
						<tr>
							<th class="obrigatorio">Inventário:</th>
							<td>
								<h:selectOneMenu id="outputInventarios" value="#{registraMateriaisInventarioMBean.inventario}">
									<f:selectItem itemLabel="-- SELECIONE --" itemValue="-1" />
									<f:selectItems value="#{registraMateriaisInventarioMBean.inventarios}"/>
								</h:selectOneMenu>
							</td>
						</tr>
						...
						<%-- <tr>
							...
							<td><t:inputCalendar id="Data" renderAsPopup="true" renderPopupButtonAsImage="true" size="10" maxlength="10" value="#{registraMateriaisInventarioMBean.data}" onkeypress="return formataData(this,event)" popupDateFormat="dd/MM/yyyy" /></td>
						</tr>
						<tr>
							<th>Turno da Consulta:</th>
							<td>
								<h:selectOneRadio value="#{registraMateriaisInventarioMBean.turno}">
					...
					<td colspan="2" align="center">
						<ufrn:checkRole papeis="<%= new int[] {SigaaPapeis.BIBLIOTECA_SETOR_INFO_E_REF_BIBLIOTECARIO
							...
							, SigaaPapeis.BIBLIOTECA_SETOR_CIRCULACAO, SigaaPapeis.BIBLIOTECA_ADMINISTRADOR_GERAL}  %>">
								<h:commandButton value="Registrar Materiais" action="#{registraMateriaisInventarioMBean.registrarMateriais}" id="botaoRegistrarMateriais" /> 
						</ufrn:checkRole>
						
						<h:commandButton value="Cancelar" action="#{registraMateriaisInventarioMBean.cancelar}" onclick="#{confirm}" immediate="true" id="cancelarRegistroConsulta" />
