...
<f:view>
	...
	<h:form id="formulario">
		...
		<h2> <ufrn:subSistema /> &gt; Empréstimos Ativos</h2>
		...
		<c:set var="_infoUsuarioCirculacao" value="${listaEmprestimosAtivosUsuarioMBean.infoUsuario}" scope="request" />
		<%@include file="/biblioteca/circulacao/dadosUsuarioBiblioteca.jsp"%>



		<%--  Mostra os emprestios ativos do usuario acima   --%>
	
		<c:if test="${not empty listaEmprestimosAtivosUsuarioMBean.emprestimosAtivos  }">
			...
			<div id="divDadosEmprestimos" style="margin-top:30px">
				...
				<table class="listagem" >
					<caption> Empréstimos Ativos(${fn:length(listaEmprestimosAtivosUsuarioMBean.emprestimosAtivos)}) </caption>
					<thead>
					<tr>
					...
					<tbody>
						...
						<c:forEach items="#{listaEmprestimosAtivosUsuarioMBean.emprestimosAtivos}" var="emprestimo" varStatus="status">
							...
							<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar"}">
								...
								<td width="2%">	
									...
									<ufrn:checkRole papeis="<%= new int[] { SigaaPapeis.BIBLIOTECA_SETOR_CIRCULACAO_BIBLIOTECARIO, SigaaPapeis.BIBLIOTECA_ADMINISTRADOR_GERAL}  %>">
										...
										<h:commandLink  action ="#{estornaEmprestimo.estornar}"
											onclick="return confirm('Confirma estorno do empréstimo? ');">
		
											<h:graphicImage url="/img/biblioteca/estornar.gif"
												style="border:none"
												title="Clique aqui para Estornar esse Empréstimo" />
	
											<f:param name="idEmpresestimoVaiSerEstornado" value="#{emprestimo.id}"/>
											<f:param name="idUsuarioEmprestimo" value="#{listaEmprestimosAtivosUsuarioMBean.infoUsuario.idUsuarioBiblioteca}"/>
					...
					<tfoot>
						<tr>
							<td colspan="8" style="text-align: center; font-weight: bold;">
								<h:commandButton value="<< Voltar" action="#{listaEmprestimosAtivosUsuarioMBean.voltar}" />
							</td>
						</tr>
					</tfoot>
		...
		<c:if test="${empty listaEmprestimosAtivosUsuarioMBean.emprestimosAtivos }">
			...
			<h:commandLink value="Voltar" action="#{listaEmprestimosAtivosUsuarioMBean.voltar}"></h:commandLink>	
