...
<f:view>
	...
	<div class="infoAltRem">
		 <h:graphicImage value="/img/adicionar.gif"style="overflow: visible;"/><h:commandLink action="#{montagem.preCadastrar}" value="Cadastrar Montagem"/><h:graphicImage value="/img/alterar.gif"style="overflow: visible;"/>: Alterar
	...
	<h:outputText value="#{montagem.create}" />
	
<c:set var="lista" value="${montagem.all}" />
<c:if test="${empty lista}">
	<br />
	<center>
	<span style="color:red;">Nenhuma Montagem Encontrada.</span>
	</center>
</c:if>
<c:if test="${not empty lista}">

	<table class=listagem style="width:100%" border="1">
		<caption class="listagem">Lista de Montagens</caption>
		<thead>
			<tr>
				<td></td>
				<td>Título</td>
		...
		<h:form>
		<c:forEach items="#{montagem.all}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				...
				<td width=20>
					<h:commandLink action="#{montagem.atualizar}" >
						<h:graphicImage value="/img/alterar.gif" style="overflow: visible;" title="Alterar"/>
						<f:param name="id" value="#{item.id}"/>
					</h:commandLink>
				</td>						
				<td width=25>
					<h:commandLink action="#{montagem.remover}"  onclick="#{confirmDelete}">
...
<h:outputText value="#{montagem.dropList}" />
</f:view>
