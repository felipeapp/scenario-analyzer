	...
	<table class="listagem">
		...
		<c:forEach items="#{turmasVirtuais}" var="t" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				...
				<td>
					<h:commandLink action="#{arquivoUsuario.selecionarTurma}">
						<f:param name="idTurma" value="#{t.id}" />
						<h:graphicImage value="/img/avancar.gif"/>
					</h:commandLink>
				</td>
			</tr>
		<%--<a href="${ctx}/associarArquivo.do?dispatch=escolher&id=${ t.id }&idArquivo=${ idArquivo }"><img src="/sigaa/img/avancar.gif"/></a>--%>
		</c:forEach>
	</table>
</c:if>
</h:form>
