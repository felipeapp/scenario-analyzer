...
<f:view>
	...
	<h:form id="formBusca">
			...
			<table class="listagem" width="100%" cellpadding="3px">
				<caption>Manifestações Encontradas(${analiseManifestacaoServidor.totalManifestacoes })</caption>
				<c:if test="${not empty analiseManifestacaoServidor.manifestacoesCopiadas }">
					<tr><td colspan="7" class="subFormulario" style="text-align: center;">Minhas Manifestações(${ fn:length(analiseManifestacaoServidor.manifestacoes) })</td></tr>
				</c:if>
					...
					<tbody>
						<c:forEach items="#{analiseManifestacaoServidor.manifestacoes }" var="manifestacao" varStatus="status">
							<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
								...
								<th>
									<h:commandButton image="/img/view.gif" title="Visualizar Detalhes da Manifestação" actionListener="#{analiseManifestacaoServidor.detalharManifestacaoBuscada }">
				...
				<c:if test="${not empty analiseManifestacaoServidor.manifestacoesCopiadas }">
					<tr><td colspan="7" class="subFormulario" style="text-align: center;">Manifestações Copiadas Para Mim(${ fn:length(analiseManifestacaoServidor.manifestacoesCopiadas) })</td></tr>
					<tr><td>
						<table class="listagem" width="100%" cellpadding="3px">
							<thead>
								<tr>
