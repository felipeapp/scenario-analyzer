...
<f:view>
	...
	<h:form id="formBusca">
			...
			<table class="listagem" width="100%" cellpadding="3px">
				<caption>Manifestações Encontradas(${analiseManifestacaoDiscente.totalManifestacoes })</caption>
				<c:if test="${not empty analiseManifestacaoDocente.manifestacoesCopiadas }">
					<tr><td colspan="7" class="subFormulario" style="text-align: center;">Minhas Manifestações(${ fn:length(analiseManifestacaoDiscente.manifestacoes) })</td></tr>
				</c:if>
				<c:if test="${empty analiseManifestacaoDiscente.manifestacoes }">
				...
				<c:if test="${not empty analiseManifestacaoDiscente.manifestacoes }">
					<tr><td>
					<table class="listagem" width="100%" cellpadding="3px">
					<thead>
						<tr>
					...
					<tbody>
						<c:forEach items="#{analiseManifestacaoDiscente.manifestacoes }" var="manifestacao" varStatus="status">
							<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
								...
								<th>
									<h:commandButton image="/img/view.gif" title="Visualizar Detalhes da Manifestação" actionListener="#{analiseManifestacaoDiscente.detalharManifestacaoBuscada }">
				...
				<c:if test="${not empty analiseManifestacaoDiscente.manifestacoesCopiadas }">
					<tr><td colspan="8" class="subFormulario" style="text-align: center;">Manifestações Copiadas Para Mim(${ fn:length(analiseManifestacaoDiscente.manifestacoesCopiadas) })</td></tr>
					<tr><td>
						<table class="listagem" width="100%" cellpadding="3px">
							<thead>
								<tr>
							...
							<tbody>
								<c:forEach items="#{analiseManifestacaoDiscente.manifestacoesCopiadas }" var="manifestacao" varStatus="status">
									<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
										<th style="text-align: center;"><ufrn:format  valor="${manifestacao.dataCadastro }" type="datahora"/></th>
										<th style="padding-right: 10%; text-align: right;">${manifestacao.numeroAno }</th>
