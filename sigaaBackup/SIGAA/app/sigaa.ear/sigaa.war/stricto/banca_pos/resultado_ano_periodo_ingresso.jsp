<%-- Resultado da consulta de bancas de defesas ordenado por ano-período de ingresso --%>
<table class="listagem">
	...
	<tbody>
		...
		<c:set var="grupoIngresso" value="" />
		<c:set var="grupoDiscente" value="" />
		...
		<c:forEach items="#{consultarDefesaMBean.listaBancaPos}" var="item" varStatus="status">
				...
				<c:set var="loopIngresso" value="${item.dadosDefesa.discente.anoIngresso}.${item.dadosDefesa.discente.periodoIngresso}"/>
				<c:if test="${loopPrograma != grupoPrograma}">
					<tr>
				...
				<c:if test="${loopIngresso != grupoIngresso}">
					<c:set var="index" value="${index + 1}" />
					<tr>
						<td colspan="6" class="subFormulario">
							<c:if test="${not empty item.data}">
								Discentes Ingressantes em ${ loopIngresso }
							</c:if>
						</td>
					</tr>
					<c:set var="grupoIngresso" value="${loopIngresso}"/>
					<c:set var="index" value="0" />
					<c:set var="grupoDiscente" value="" />
				</c:if>
				...
				<tr class="${index % 2 != 0 ? 'linhaPar' : 'linhaImpar' }">
					...
					<td>
						<h:commandLink action="#{ consultarDefesaMBean.visualizarDadosDefesa }" id="linkVisualizarDadosDefesa"> 
								<f:param name="idBancaPos" value="#{ item.id }" id="idDaBancaPos"/>
					...
					<td>
						...
						<c:if test="${consultarDefesaMBean.ehCatalogacao}">
							<h:commandLink action="#{ catalogacaoMBean.iniciarDefesa }" id="linkIniciarDefesa"> 
