...
<%-- Resultado da consulta de bancas de defesas ordenado por data --%>
<table class="listagem">
	...
	<tbody>
		...
		<c:forEach items="#{consultarDefesaMBean.listaBancaPos}" var="item" varStatus="status">
				...
				<c:set var="loopData" value="${item.dataFormatada}"/>
				...
				<c:if test="${loopData != grupoData}">
					<c:set var="index" value="${index + 1}" />
					<tr>
						<td colspan="6" class="subFormulario">
							<c:if test="${not empty item.data}">
								Bancas do dia ${ loopData }
							</c:if>
							<c:if test="${empty item.data}">
								Bancas sem Data Cadastrada
							</c:if>
						</td>
					</tr>
					<c:set var="grupoData" value="${loopData}"/>
					<c:set var="index" value="0" />
					<c:set var="grupoDiscente" value="" />
				</c:if>
