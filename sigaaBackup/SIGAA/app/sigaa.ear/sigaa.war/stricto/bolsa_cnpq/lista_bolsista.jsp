...
<f:view>
	...
	<div class="infoAltRem">
		<h:graphicImage value="/img/adicionar.gif"style="overflow: visible;"/>: <h:commandLink action="#{bolsasCnpqStrictoMBean.iniciarCadastro}" value="Cadastrar novo bolsista"/>
	...
	<c:if test="${not empty bolsasCnpqStrictoMBean.bolsistas}">
	<table class=listagem>
		...
		<c:forEach items="#{bolsasCnpqStrictoMBean.bolsistas}" var="_bolsista" varStatus="status">
			<tr class="${ status.index % 2 == 0 ? 'linhaPar' : 'linhaImpar' }">
				<td style="text-align:center;">${_bolsista.discente.matricula}</td>
				<td>${_bolsista.discente.nome}</td>
				<td style="text-align:center;"> 
					<ufrn:format type="data" valor="${_bolsista.bolsa.inicio}" /> a <ufrn:format type="data" valor="${_bolsista.bolsa.fim}" />
				</td>
				<td width="20">
					<h:commandLink styleClass="noborder" title="Alterar dados" action="#{bolsasCnpqStrictoMBean.iniciarCadastro}" id="linkAlterarDados">
						<h:graphicImage url="/img/alterar.gif"/>
						<f:setPropertyActionListener target="#{bolsasCnpqStrictoMBean.obj}" value="#{_bolsista}"/>
					</h:commandLink>
				</td>
				<td width="20">
					<h:commandLink styleClass="noborder" title="Finalizar bolsista" action="#{bolsasCnpqStrictoMBean.finalizar}" 
