...
<h:form id="form">
	<table class="formulario" style="width: 90%">
		...
		<tr>
			...
			<td class="desc linhaImpar">${moduloMBean.obj.codigo}</td>
		</tr>		

		<tr>
			<td class="desc linhaPar">Descrição</td>
			<td class="desc linhaPar">${moduloMBean.obj.descricao}</td>
		</tr>		
		
		<tr>
			<td class="desc linhaImpar">CH TOTAL MODULO</td>
			<td class="desc linhaImpar">${moduloMBean.obj.cargaHoraria} ch <span style="font-style: italic; font-weight: normal;">(Esse valor é manual e definido na criação do módulo)</span></td>
		</tr>		
		...
		<c:forEach items="#{moduloMBean.obj.moduloDisciplinas}" var="md" varStatus="status">
		...
		<c:forEach items="#{moduloMBean.obj.moduloCurriculares}" var="mc" varStatus="status">
			<tr class="${status.index % 2 == 0 ? 'linhaImpar' : 'linhaPar'}">
