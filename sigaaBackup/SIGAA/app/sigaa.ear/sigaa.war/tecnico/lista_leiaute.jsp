...
<f:view>
	...
	<h:form>
		...
		<caption>Lista de Leiaute de Arquivos para Importação (${fn:length(importaAprovadosTecnicoMBean.listaLeiautes)})</caption>
			<thead>
				<tr>
					<th>Descrição</th>
			...
			<c:forEach items="#{importaAprovadosTecnicoMBean.listaLeiautes}" var="item"
				varStatus="status">
				<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
					...
					<td>
						<h:commandLink title="Visualizar Leiaute" style="border: 0;" action="#{importaAprovadosTecnicoMBean.view}" >
							<f:param name="id" value="#{item.id}" />
							<h:graphicImage url="/img/view.gif" alt="Visualizar Leiaute" />
						</h:commandLink>
					</td>
					<td>
						<h:commandLink title="Alterar Leiaute" style="border: 0;" action="#{importaAprovadosTecnicoMBean.atualizar}" >
							<f:param name="id" value="#{item.id}" />
							<h:graphicImage url="/img/alterar.gif" alt="Alterar Leiaute" />
						</h:commandLink>
					</td>
					<td>
						<h:commandLink title="Duplicar Leiaute" style="border: 0;" action="#{importaAprovadosTecnicoMBean.view}" >
							<f:param name="id" value="#{item.id}" />
							<h:graphicImage url="/img/biblioteca/duplicar.png" alt="Duplicar Leiaute" />
						</h:commandLink>
					</td>
					<td>
						<h:commandLink title="Inativar Leiaute" style="border: 0;" action="#{importaAprovadosTecnicoMBean.inativar}" 
			...
			<tfoot>
				<tr>
					<td colspan="7" style="text-align: center;">
						<h:commandButton immediate="true" value="Cancelar" action="#{importaAprovadosTecnicoMBean.cancelar}" onclick="#{ confirm }"/>
					</td>
				</tr>
			</tfoot>
		</table>
