...
import br.ufrn.sigaa.biblioteca.controle_estatistico.dao.RelatorioTotalPeriodicosPorClasseDAO;
import br.ufrn.sigaa.biblioteca.controle_estatistico.dominio.DadosRelatorioTotalPeriodicosPorClassificacao;

/**
 * Bean que cuida da geração do relatório de total de periódicos por classe CDU ou Black.
 *
 * @author Bráulio (refactoring)
 */
@Component("relatorioTotalPeriodicosPorClasseMBean")
@Scope("request")
public class RelatorioTotalPeriodicosPorClasseMBean extends AbstractRelatorioBibliotecaMBean {

	/** Caminho para a página do relatório. */
	private static final String PAGINA_RELATORIO = "/biblioteca/controle_estatistico/relatorioPeriodicosClasseAcervo.jsp";
	
	/** Os resultados parcial da primeira consulta */
	private List<Object[]> resultadosParcial;
	
	/** Os resultados retornado para o usuário */
	private List<DadosRelatorioTotalPeriodicosPorClassificacao> resultados;
	
	public RelatorioTotalPeriodicosPorClasseMBean(){
	...
	public void configurar() {
		titulo = "Relatório de Total de Periódicos por Classificação";
		descricao = "<p>Este relatório lista a quantidade de periódicos no acervo, agrupados pela classificação bibliográfica utilizada, periódicos nacionais e internacionias " +
				"e correntes ou não correntes .</p>" +
				"<p><strong>Observação:</strong> São recuperados apenas os fascículos cadatrados dentro do período informado, contudo no cálculo dos periódicos correntes e não correntes é considerado a data de registro no sistema do último fascículo da assinatura, independetemente se esse fascículo está dentro do período informado ou não. </p>";
	...
	public String gerarRelatorio() throws DAOException, SegurancaException {
		...
		RelatorioTotalPeriodicosPorClasseDAO dao = getDAO(RelatorioTotalPeriodicosPorClasseDAO.class);
		 ...
		 * Object[3] = a classificação
		 */
		resultadosParcial = dao.
				findPeriodicosClasseAcervoByTombamentoBiblioteca(UFRNUtils.toInteger(variasBibliotecas), 
		...
		resultados = new ArrayList<DadosRelatorioTotalPeriodicosPorClassificacao>();
	...
	private boolean verificaSePeriodicoEstarCorrente(Short qtdMeses, Date dataUltimoRegistro) {
		if(qtdMeses != null && dataUltimoRegistro != null){  //  A assinatura tem a frequência, então vamos calcular
			...
			if(dataLimite.after(hoje)){ // Corrente !!!!!
				return true;
			}
	...
	private void agrupaResultados(Object[] dados, boolean corrente, boolean interNacional) {
		...
		DadosRelatorioTotalPeriodicosPorClassificacao resultadoTemp = new  DadosRelatorioTotalPeriodicosPorClassificacao( (String) dados[3] );
	...
	public List<DadosRelatorioTotalPeriodicosPorClassificacao> getResultados() { return resultados; }
