...
/**
 * MBean responsável por emitir os certificados dos discentes com plano 
 * de trabalho nos projetos Integrados
 * 
 * @author Jean Guerethes
 */
@Component @Scope("session")
public class CertificadoPlanoTrabalhoIntegradoMBean extends 
	...
	public static final String DECLARACAO = "plano_associados";

	public CertificadoPlanoTrabalhoIntegradoMBean() {
		clear();
	}
		...
		try {
			...
			if ( !verificando ) {
				comprovante = geraEmissao(
						...
						SubTipoDocumentoAutenticado.CERTIFICADO_PLANO_ASSOCIADOS, true);
			}
			...
			getCurrentResponse().addHeader(
					"Content-Disposition",
					"attachment; filename=CERTIFICADO_ASSOCIADAS.pdf");
	...
	public boolean validaDigest(EmissaoDocumentoAutenticado comprovante) {
		...
		try {
			...
			if (comprovante.getSubTipoDocumento().equals(SubTipoDocumentoAutenticado.CERTIFICADO_PLANO_ASSOCIADOS) ) {
