 ...
 *
 */
public class DesktopTokenAuthHttpInvokerProxyFactoryBean extends HttpInvokerProxyFactoryBean implements ApplicationContextAware {

	private Integer sistema;
	
	private ApplicationContext ac;
	
	public DesktopTokenAuthHttpInvokerProxyFactoryBean() {
		setHttpInvokerRequestExecutor(new SimpleHttpInvokerRequestExecutor() {
			@Override
			protected HttpURLConnection openConnection( HttpInvokerClientConfiguration config) throws IOException {
				TokenAutenticacaoRemoteService service = (TokenAutenticacaoRemoteService) ac.getBean("tokenAutenticacaoInvoker");
				Object[] result = service.registerToken(getRegistroEntrada(), getSistema(), getInfo());
				String url = getServiceUrl() + "?key=" + result[1] + "&id=" + result[0]; // mudar para id
