...
public aspect AspectJUnit3Scenario {
	...
	private pointcut jUnit3ExecutionFlow() :
		cflow(within(junit.framework.TestCase+) && execution(* test*(..)))
		&& (execution(* *(..)) || execution(*.new(..)));
	
	Object around() : jUnit3ExecutionFlow() && !executionIgnored() {
		long begin, end;
		...
		if (nodes_stack.empty()) {
			RuntimeScenario scenario = new RuntimeScenario(
					AspectsUtil.getEntryPointName(member), node, AspectsUtil.getContextParameterMap());
	...
	after() throwing(Throwable t) : jUnit3ExecutionFlow() && !executionIgnored()  {
		Member member = AspectsUtil.getMember(thisJoinPoint.getSignature());
