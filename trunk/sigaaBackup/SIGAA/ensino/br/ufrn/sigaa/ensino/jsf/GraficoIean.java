...
/**
 * Classe para renderizar o gráfico de IEAN no relatório 
...
public class GraficoIean implements DatasetProducer, ChartPostProcessor, Serializable {
	...
	public Object produceDataset(Map params) throws DatasetProduceException {
		double iea = Double.parseDouble(params.get("iea").toString());
		double maiorIea = Double.parseDouble(params.get("maiorIea").toString());
		double menorIea = Double.parseDouble(params.get("menorIea").toString());
		...
		XYSeriesCollection dataset = (XYSeriesCollection) DatasetUtilities.sampleFunction2D(gaussian, menorIea, maiorIea, 10000, "Relação com a média");
		XYSeries serie = (XYSeries) UFRNUtils.deepCopy(dataset.getSeries(0));
		serie.setKey("Distribuição estatística do IEA");

		double maior = Math.max(iea, mean);
		double menor = Math.min(iea, mean);
