 ...
 * na seleção de discentes para o IMD.
 * 
...
public class ConvocacaoProcessoSeletivoTecnicoIMDMBean extends SigaaAbstractController<ConvocacaoProcessoSeletivoTecnico> {
	...
	/**Lista das opções polo grupo para o combo box*/
	private Collection<SelectItem> polosCombo = new ArrayList<SelectItem>();
	
	/**Objeto que representa a opção polo grupo selecionada*/
	private OpcaoPoloGrupo opcaoSelecionada = new OpcaoPoloGrupo();
	
	/**Representa a quantidade de vagas que a opção selecionada possui em um determinado processo seletivo*/
	private Long qtdVagasOpcaoSelecionada = (long) 0;
	
	/**Representa a quantidade de convocados que a opção selecionada possui em um determinado processo seletivo*/
	private Long qtdConvocadosOpcaoSelecionada = (long) 0;

	/**Objeto que representa o PS selecionado para a convocação*/
	private ProcessoSeletivoTecnico psSelecionado = new ProcessoSeletivoTecnico();
	
	/**Atributo que armazena a quantidade de suplentes de um determinado pólo*/
	private Long qtdSuplentePolo = (long) 0;
	
	/**Atributo que armazena a quantidade de geral de vagas disponíveis*/
	...
	public String cadastrarConvocacoesImportacaoTecnico () throws SegurancaException, ArqException, NegocioException {
		...
		if (obj.getOpcao().getId() == 0) {
			erros.addErro("Selecione uma Opção - Pólo - Grupo");
		}
	...
	public String cadastrarConvocacoesImportacaoTecnicoNew () throws SegurancaException, ArqException, NegocioException {
		...
		if (obj.getProcessoSeletivo().getId() == 0) {
			erros.addErro("Processo Seletivo: Campo obrigatório não informado.");
		}
		
		if (obj.getOpcao().getId() == 0) {
			erros.addErro("Pólo / Grupo: Campo obrigatório não informado.");
		}
		
		if (obj.getQuantidadeDiscentesSemReserva() == null || obj.getQuantidadeDiscentesSemReserva() == 0) {
			obj.setQuantidadeDiscentesSemReserva(null);
			erros.addErro("Quantidade de discentes a serem convocados: Campo obrigatório não informado.");
		}
		...
		if( hasErrors() ) {
			...
			try {
				...
				if (resumoConvocacao != null && !resumoConvocacao.isEmpty()){
					...
					addMensagemInformation("Os discentes que informaram o e-mail no processo seletivo foram notificados sobre a convocação.");
				}
			} catch (NegocioException e ){
				addMensagens (e.getListaMensagens());
				return redirectMesmaPagina();
	...
	/**
	 * Método GET para o atributo gruposCombo. Também efetua o preenchimento do atributo caso esteja vazio
	 * 
	 * @param
	 * @return
	 * @throws DAOException
	...
	/**
	 * Método GET para o atributo polosCombo. Também efetua o preenchimento do atributo caso esteja vazio
	 * 
	 * @param
	 * @return
	 * @throws DAOException
	...
	/**
	 * Carrega os dados relacionados as vagas para a convocação
	 * 
	 * Método chamado pela(s) seguinte(s) JSP(s):
	 * <ul>
	 * <li>/sigaa.war/metropole_digital/convocacao/form_cadastramento.jsp</li>
			...
			if( hasErrors() ) {
				...
				if(qtdVagasOpcaoSelecionada != null) {
					...
					if(qtdSuplentePolo > 0) {
						if(qtdDisponiveis <= 0) {
							addMensagemWarning("A opção - pólo - grupo selecionada não possui nenhuma vaga disponível.");
							qtdDisponiveis = (long) 0;
						}
						
						return forward("/metropole_digital/convocacao/form_convocacao_importacao.jsp");
					} else {
						addMensagemErro("Não há nenhum candidato suplente para os critérios informados.");
						return forward("/metropole_digital/convocacao/form_inicial.jsp");
					}
				}
				else {
					addMensagemErro("O processo seletivo não possui nenhum grupo de reserva de vagas cadastrado.");
					return forward("/metropole_digital/convocacao/form_inicial.jsp");
				}
			}
