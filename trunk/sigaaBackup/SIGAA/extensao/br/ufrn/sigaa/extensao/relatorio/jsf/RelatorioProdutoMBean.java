...
/**
 * Mbean responsável por controlar o cadastro de relatórios de produtos de extensão
 * 
 * @author Ilueny Santos
 * 
 */
@Scope("session")
@Component("relatorioProdutoMBean")
public class RelatorioProdutoMBean extends SigaaAbstractController<RelatorioProdutoExtensao> {
	...
	/** Construtor padrão. */
	public RelatorioProdutoMBean() {
		obj = new RelatorioProdutoExtensao();
	}
	 ...
	 * /extensao/RelatorioProduto/lista.jsp
	 *  
	 * @return
	 * @throws ArqException
	 */
	public void preAdicionarRelatorio(AtividadeExtensao atividade, Date dataLimite) throws ArqException {
		...
		prepareMovimento(SigaaListaComando.ENVIAR_RELATORIO_PRODUTO_EXTENSAO);
		...
		obj.getAtividade().setOrcamentosDetalhados(getGenericDAO().findByExactField(OrcamentoDetalhado.class, "projeto.id", obj.getAtividade().getProjeto().getId()));		
		obj.setDataCadastro(new Date());
		obj.setRegistroEntradaCadastro(getUsuarioLogado().getRegistroEntrada());
		obj.setTipoRelatorio(tipoRelatorio);
		obj.setPublicoRealAtingido(atividade.getPublicoAtendido());
		obj.setAtivo(true);

		// Criando detalhamento de recursos do produto (recursos utilizados)
		...
		for (OrcamentoConsolidado oc : atividade.getOrcamentosConsolidados()) {
			...
			recurso.setRelatorioAcaoExtensao(obj);
			recursos.add(recurso);
		}

		obj.setDetalhamentoRecursos(recursos);
		setReadOnly(false);
	...
	public void preAlterarRelatorio(AtividadeExtensao atividade, Date dataLimite) throws SegurancaException {
		...
		try {
			...
			obj = dao.findByPrimaryKey(getParameterInt("idRelatorio"),	RelatorioProdutoExtensao.class);
			obj.setRegistroEntradaCadastro(getUsuarioLogado().getRegistroEntrada());
	...
	public String preRemoverRelatorio() throws SegurancaException {
		...
		return forward(ConstantesNavegacao.RELATORIO_PRODUTO_DELETE);

	}

	/**
	 * Método usado para realizar submissão de relatórios de Produtos.
	 *
	...
	/**
	 * Método usado para fazer o envio de um relatório (Produto).
	 * 
	 * Método chamado pela(s) seguinte(s) JSP(s):
	 * /extensao/RelatorioProduto/form.jsp
	 * 
	...
	public String enviar() throws ArqException {
		...
		try {
			...
			submeter(SigaaListaComando.ENVIAR_RELATORIO_PRODUTO_EXTENSAO);
	...
	/**
	 * Remover o relatório de produto de extensão
	 * 
	 * Método chamado pela(s) seguinte(s) JSP(s):
	 * /extensao/RelatorioProduto/form.jsp
	 * 
...
//		return mbean.iniciarCadastroRelatorio();
	 ...
	 * Método chamado pela(s) seguinte(s) JSP(s):
	 * <ul>
	 * 	<li>sigaa.war/extensao/RelatorioProduto/form.jsp</li>
	 * </ul>
	 * @throws DAOException
	 */
	public String importarDadosRelatorioParcial() throws DAOException{
		...
		if ((relatorios != null) && (!relatorios.isEmpty())) {
			...
			RelatorioProdutoExtensao relatorioParcial = dao.findByPrimaryKey(relatorios.iterator().next().getId(), RelatorioProdutoExtensao.class);
