...
/**
 * Controller responsável pelas operações de registro de diplomas antigos (anteriores ao registro eletrônico no SIGAA)
...
public class RegistroDiplomaAntigoMBean extends	SigaaAbstractController<RegistroDiploma> implements OperadorDiscente{
	...
	/** Responsáveis pela assinatura no diploma antigo no momento do Registro. */
	...
	public String submeterDadosGerais() throws SegurancaException, ArqException, NegocioException {
		...
		if (!livro.isLivroAntigo()) {
			addMensagemErro("O livro utilizado para registro é para novos registros. Selecione um livro para registro de diplomas antigos.");
			return null;
		} else {
	...
	public String submeterAssinaturas() throws SegurancaException, ArqException, NegocioException {
		...
		if(hasErrors())
			return null;
		// verifica se não há registro de assinaturas cadastrado para os valores informados
		...
		for (ResponsavelAssinaturaDiplomas ass : getGenericDAO().findAll(ResponsavelAssinaturaDiplomas.class)) {
			if (EqualsUtil.testTransientEquals(assinaturaDiploma, ass, atributos)) {
				addMensagemErro("Há uma configuração de assinaturas na lista com os mesmos valores informados. Por favor, selecione o item correspondente na lista.");
				return null;
			}
		}
		...
		if (registroAntigo != null && !operacaoAlterar) {
			...
			if (this.obj.getDataColacao() != null) {
				...
				if (ano * 10 + semestre >= anoInicioRegistro * 10 + semestreInicioRegistro) {
					addMensagemErro("Este dicente não pode ter o diploma registrado como antigo pois sua data de colação é anterior a "
