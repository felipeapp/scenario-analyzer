...
<f:view>
		...
		<table class="tabelaRelatorioBorda" style="width:100%;margin-top: 40px;">
			...
			<tr>
				...
				<c:forEach var="totalPorArea" items="#{_abstractRelatorioBiblioteca.quantidadeAnteriorTitulo}">
					<th style="text-align: center; width: 100px;"> ${totalPorArea.key}</th>
				</c:forEach>
			...
			<tr>
				<td></td>
				<c:forEach var="totalPorArea" items="#{_abstractRelatorioBiblioteca.quantidadeAnteriorTitulo}">
					<td style="width:  100px;"> ${totalPorArea.value}</td>
							...
							<table style="width: 100%; border: 0px;">
								<tr>
									<c:forEach var="dadosInternos" items="#{dadosPorAgrupamento.dadosInternos}" >
										<th style="width: 50px;text-align: center;">${dadosInternos.descricaoArea}</th>
									</c:forEach>
								</tr>
								<tr>
									<c:forEach var="dadosInternos" items="#{dadosPorAgrupamento.dadosInternos}" >
		...
		<c:forEach var="dadosDoCrescimentoMateriais" items="#{_abstractRelatorioBiblioteca.resultadoCrescimentoMaterial}" >
			...
			<c:forEach var="dadosPorMes" items="#{dadosDoCrescimentoMateriais.dadosInternos}" >
				...
				<tr>
					...
					<c:forEach var="siglaArea" items="#{siglasAreasCNPq}">                         
						<th style="width: 80px; max-width: 80px; min-width: 80px; text-align:center;">${ siglaArea.sigla }</th>
					</c:forEach>
					<th style="width: 100px; max-width: 100px; min-width: 100px; font-weight: bold;text-align: center;">Total</th>   <%-- Total por agrupamento --%>   
				</tr>
								...
								<tbody>
									<tr>
										<c:forEach var="dadosInternos" items="#{dadosPorAgrupamento.dadosInternos}"  varStatus="loop2">
											<td style="width: 80px; max-width: 80px; min-width: 80px; margin: 0px; padding: 0px;">
												<c:if test="${_abstractRelatorioBiblioteca.dadoBooleano}">
													${dadosInternos.quantidadeAnterior} + 
												</c:if>
												<span style="color:blue;">${dadosInternos.quantidade} </span>  
				...
				<%-- Imprime do rodapé um total para cada área. --%>   
				<tr>
					<td style="font-weight: bold; text-align: center;">	Total</td>
					<c:forEach var="totalPorArea" items="#{dadosPorMes.totaisPorArea}">
						<td style="font-weight: bold;"> ${totalPorArea.value}</td>
					</c:forEach>
	...
	<div style="text-align: center; margin: 10px;">
		...
		<cewolf:chart id="grafico" type="stackedverticalbar3d"
				xaxislabel="Área CNPq" yaxislabel="Fascículos" showlegend="true">
			<cewolf:data>
