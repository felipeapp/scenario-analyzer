...
<table width="90%" class="formulario">
	...
	<c:if test="${catalogacaoMBean.tipoCatalogacaoBibliografica}">
		<caption>003 ( ${catalogacaoMBean.obj.formatoMaterial.descricaoCompleta} )</caption>
	</c:if>

	<c:if test="${catalogacaoMBean.tipoCatalogacaoAutoridade}">
		<caption>003 Autoridade </caption>
	</c:if>

	<tr>
		<th class="alinhamentoDireita">
		IDENTIFICADOR DO NÚMERO DE CONTROLE (NR):
		</th>
		<td>
			<h:inputText value="#{catalogacaoMBean.dadosCamposControle}" autocomplete="off" size="50" maxlength="100">
	...
	<tfoot>
		<tr>
			<td colspan="2">
				...
				<h:commandButton value="<< Página Catalogação" action="#{catalogacaoMBean.submeterCampo003}">
				...
				<c:if test="${catalogacaoMBean.adicionandoCamposDeControle == true}">
					<h:commandButton value=" Adicionar Campo >>" action="#{catalogacaoMBean.submeterCampo003}"/>
				</c:if>
				
				<c:if test="${catalogacaoMBean.editandoCamposDeControle == true}">
					<h:commandButton value=" Atualizar Campo >>" action="#{catalogacaoMBean.submeterCampo003}"/>
				</c:if>
