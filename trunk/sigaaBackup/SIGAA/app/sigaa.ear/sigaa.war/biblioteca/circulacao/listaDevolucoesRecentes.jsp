...
<f:view>
	...
	<h:form id="formulario">
		...
		<h2> <ufrn:subSistema /> &gt; Devoluções Recentes</h2>
		...
		<c:set var="_infoUsuarioCirculacao" value="${listaDevolucoesRecentesUsuarioMBean.infoUsuario}" scope="request" />
		<%@include file="/biblioteca/circulacao/dadosUsuarioBiblioteca.jsp"%>



		<%--  Mostra as devoluções recentes do usuário acima   --%>
	
		<c:if test="${not empty listaDevolucoesRecentesUsuarioMBean.devolucoesRecentes  }">
			...
			<div id="divDadosDevolucoes" style="margin-top:30px">
				
				<table class="listagem" >
					<caption> Devoluções Recentes(${fn:length(listaDevolucoesRecentesUsuarioMBean.devolucoesRecentes)}) </caption>
					<thead>
					<tr>
						<th width="15%" style="text-align:left"> Código de Barras</th>
						<th width="15%" style="text-align:center"> Data do Emprestimo</th>
						<th width="15%" style="text-align:center"> Data da Devolução </th>
					...
					<tbody>
						...
						<c:forEach items="#{listaDevolucoesRecentesUsuarioMBean.devolucoesRecentes}" var="emprestimo" varStatus="status">
							...
							<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar"}">
								...
								<td width="20%" style="text-align:center">
									<h:outputText value="#{emprestimo.dataDevolucao}" converter="convertData"></h:outputText>
								</td>
								<td width="20%" style="text-align:center">
								...
								<td width="2%">	
									...
									<ufrn:checkRole papeis="<%= new int[] { SigaaPapeis.BIBLIOTECA_SETOR_CIRCULACAO_BIBLIOTECARIO, SigaaPapeis.BIBLIOTECA_ADMINISTRADOR_GERAL}  %>">
										...
										<h:commandLink  action ="#{estornaDevolucaoMBean.estornar}"
											onclick="return confirm('Confirma estorno da devolução? ');">
		
											<h:graphicImage url="/img/biblioteca/estornar.gif"
												style="border:none"
												title="Clique aqui para Estornar essa Devolução" />
	
											<f:param name="idEmprestimo" value="#{emprestimo.id}"/>
											<f:param name="idUsuarioEmprestimo" value="#{listaDevolucoesRecentesUsuarioMBean.infoUsuario.idUsuarioBiblioteca}"/>
					...
					<tfoot>
						<tr>
							<td colspan="8" style="text-align: center; font-weight: bold;">
								<h:commandButton value="<< Voltar" action="#{listaDevolucoesRecentesUsuarioMBean.voltar}" />
							</td>
						</tr>
					</tfoot>
		...
		<c:if test="${empty listaDevolucoesRecentesUsuarioMBean.devolucoesRecentes }">
			<div style="margin-top:30px;color:red;text-align:center">	
				Usuário Não Possui Devoluções Recentes	
			</div>
			
			<div id="divLinkVoltar" style="width:70%; margin-left:auto;margin-right:auto;margin-top:20px; text-align:center;">
	
			<h:commandLink value="Voltar" action="#{listaDevolucoesRecentesUsuarioMBean.voltar}"></h:commandLink>	
