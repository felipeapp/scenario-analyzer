...
<f:view>
	...
	<h2><ufrn:subSistema /> > Situação de Curso Hábil</h2>
	<h:outputText value="#{situacaoCursoHabil.create}" />
	...
	<center>
			...
			<div class="infoAltRem">
				<h:form>
					...
					<h:commandLink action="#{situacaoCursoHabil.preCadastrar}" value="Cadastrar"/>
	...
	<table class=listagem>
		<tr>
			<caption class="listagem">Lista de Situações de Curso Hábil</caption>
			<thead>
				<td>Descrição</td>
				<td></td>
				<td></td>
		...
		<c:forEach items="${situacaoCursoHabil.allPaginado}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				...
				<td width=20>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						...
						action="#{situacaoCursoHabil.atualizar}" /></h:form>
				</td>
				<td width=25>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/delete.gif" alt="Remover"
						action="#{situacaoCursoHabil.remover}" onclick="#{confirmDelete}"/></h:form>
				</td>
			</tr>
		</c:forEach>
	</table>
