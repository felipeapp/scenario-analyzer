...
<f:view>
	...
	<h:outputText value="#{unidadeFederativa.create}" />
	...
	<center>
			...
			<div class="infoAltRem">
				<h:form>
					...
					<h:commandLink action="#{unidadeFederativa.preCadastrar}" value="Cadastrar"/>
				</h:form>
			</div>
	</center>
		
	<table class=listagem>
	  <caption class="listagem">Lista de Unidades Federativas</caption>
	    <thead>
			<tr>
				<td>País</td>
				<td>Descrição</td>
		...
		<c:forEach items="${unidadeFederativa.allPaginado}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				<td>${item.pais.nome}</td>
				<td>${item.descricao}</td>
				<td align="center">${item.sigla}</td>
				<td>${item.capital.nome}</td>
			</tr>
		</c:forEach>
	</table>
