...
<f:view>
	...
	<h:outputText value="#{tipoEtnia.create}" />
	...
	<center>
			...
			<div class="infoAltRem">
				<h:form>
					...
					<h:commandLink action="#{tipoEtnia.preCadastrar}" value="Cadastrar"/>
	...
	<table class=listagem>
	  <caption class="listagem">Lista de Tipos de Etnia</caption>
		<thead>
			<tr>
				<td>Descrição</td>
				<td></td>
		...
		<c:forEach items="${tipoEtnia.allPaginado}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				<td>${item.descricao}</td>
				<td width=20>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/alterar.gif" value="Alterar"
						action="#{tipoEtnia.atualizar}" /></h:form>
				</td>
				<td width=25>
					<h:form><input type="hidden" value="${item.id}" name="id" /> <h:commandButton
						image="/img/delete.gif" alt="Remover"
						action="#{tipoEtnia.remover}" onclick="#{confirmDelete}" /></h:form>
				</td>
			</tr>
		</c:forEach>
	</table>
