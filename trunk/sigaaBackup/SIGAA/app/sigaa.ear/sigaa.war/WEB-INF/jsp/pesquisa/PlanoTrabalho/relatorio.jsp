...
<html:form action="/pesquisa/planoTrabalho/consulta" method="post">
	...
	<table class="formulario" align="center" style="width: 99%">
		...
		<tfoot>
			<tr>
				<td colspan="3">
					<html:button dispatch="buscar" value="Buscar"/>
...
<br/><br/>
	...
	<div class="infoAltRem">
		...
		<html:img page="/img/alterar.gif" style="overflow: visible;"/>: Alterar Plano de Trabalho
		<html:img page="/img/delete.gif" style="overflow: visible;"/>: Remover Plano de Trabalho
		<br>
		<html:img page="/img/pesquisa/indicar_bolsista.gif" style="overflow: visible;"/>: Indicar/Substituir Bolsista
		<html:img page="/img/pesquisa/remover_bolsista.gif" style="overflow: visible;"/>: Finalizar Bolsista
		<html:img page="/img/biblioteca/exemplar_edit.png" style="overflow: visible;"/>: Enviar Relatório Parcial
		<html:img page="/img/pesquisa/report_go.png" style="overflow: visible;"/>: Enviar Relatório Final
	</div>
				...
				<c:choose>
						...
						<td rowspan="2" align="left" nowrap="nowrap">&nbsp;
							<ufrn:link action="/pesquisa/planoTrabalho/wizard" param="obj.id=${plano.id}&dispatch=view">
								<img src="${ctx}/img/pesquisa/view.gif"
									alt="Visualizar Plano de Trabalho"
					...
					<c:otherwise>
						<td rowspan="2" align="center" nowrap="nowrap">
								...
								<ufrn:link action="/pesquisa/planoTrabalho/wizard" param="id=${plano.id}&dispatch=edit">
									<img src="${ctx}/img/alterar.gif"
										alt="Alterar Plano de Trabalho"
										title="Alterar Plano de Trabalho" />
								</ufrn:link>
									<ufrn:link action="/pesquisa/planoTrabalho/wizard" param="id=${plano.id}&dispatch=remove">
										<img src="${ctx}/img/delete.gif"
											alt="Remover Plano de Trabalho"
											title="Remover Plano de Trabalho" />
									</ufrn:link>
								<br/>
								<c:if test="${empty plano.membroProjetoDiscente.discente}">
									<html:link action="/pesquisa/indicarBolsista?dispatch=indicar&obj.id=${plano.id}">
										<img src="${ctx}/img/pesquisa/indicar_bolsista.gif" alt="Indicar/Substituir Bolsista" title="Indicar/Substituir Bolsista"/>
									</html:link>
								</c:if>
								<c:if test="${!empty plano.membroProjetoDiscente.discente}">
									<html:link action="/pesquisa/indicarBolsista?dispatch=remover&obj.id=${plano.id}">
										<img src="${ctx}/img/pesquisa/remover_bolsista.gif" alt="Finalizar Bolsista" title="Finalizar Bolsista"/>
									</html:link>
								</c:if>
								<c:if test="${plano.tipoBolsa.necessitaRelatorio}">
									<html:link action="/pesquisa/relatorioBolsaParcial?dispatch=popularEnvioPropesq&obj.planoTrabalho.id=${plano.id}">
										<img src="${ctx}/img/biblioteca/exemplar_edit.png" alt="Enviar Relatório Parcial" title="Enviar Relatório Parcial" />
									</html:link>
								</c:if>
								<c:if test="${plano.tipoBolsa.necessitaRelatorio}">
									<html:link action="/pesquisa/relatorioBolsaFinal?dispatch=popularEnvioPropesq&obj.planoTrabalho.id=${plano.id}">
										<img src="${ctx}/img/pesquisa/report_go.png" alt="Enviar Relatório Final" title="Enviar Relatório Final" />
									</html:link>
							</c:if>
						</td>
					</c:otherwise>
