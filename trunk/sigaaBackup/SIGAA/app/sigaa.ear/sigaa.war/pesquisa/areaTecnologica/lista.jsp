...
<f:view>
	<h2><ufrn:subSistema /> &gt; Áreas de Conhecimento Tecnológicas</h2>
	...
	<center>
			<div class="infoAltRem">
				<h:form id="formLegenda">
					...
					<h:commandLink action="#{areaConhecimentoTecnMBean.preCadastrar}" value="Cadastrar"/>&nbsp;
	...
	<table class="listagem">
		<caption class="listagem">Lista das Áreas de Conhecimento Tecnológicas</caption>
		<thead>
		<tr>
			<td>Àrea de Conhecimento</td>
		...
		<c:forEach items="#{areaConhecimentoTecnMBean.allPaginado}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				...
				<td width="20">
					<h:commandLink title="Alterar" action="#{areaConhecimentoTecnMBean.atualizar}">
						<f:param name="id" value="#{item.id}"/>
						<h:graphicImage url="/img/alterar.gif"/>
					</h:commandLink>
				</td>
				<td width="25">
					<h:commandLink title="Remover" action="#{areaConhecimentoTecnMBean.remover}" onclick="#{confirmDelete}">
						<f:param name="id" value="#{item.id}"/>
