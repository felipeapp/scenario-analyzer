...
<h2><ufrn:subSistema /> &gt; Finalização de Bolsas de Iniciação à Pesquisa</h2>

<h:form id="form">

	<div class="descricaoOperacao">
		<p><strong>Caro Gestor,</strong></p>
		<p>Esta operação lista os alunos finalizados das bolsas de modalidades pagas pela própria instituição, 
		...
		Caso identifique alguma inconsistência, como alunos aparecendo repetidos, utilize a opção para ignorar a finalização do(s) caso(s) incorreto(s).</p> 
	</div>

	<table class="formulario" width="50%">
		<caption>Busca por Finalizações de Bolsistas</caption>
		<tbody>
	...
	<c:if test="${not empty discentesBolsistasSigaa}">
		<div class="infoAltRem">
		    <h:graphicImage value="/img/view.gif" style="overflow: visible;"/>: Visualizar Resumo da Finalização/Substituição
		    <h:graphicImage url="/img/pesquisa/remover_bolsista.gif" style="overflow: visible;"/>: Ignorar Finalização de Bolsista
		</div>
	                   ...
	                   <td>
							...
							<html:link action="/pesquisa/indicarBolsista?dispatch=resumo&obj.id=${idPlanoTrabalho}">
								<img src="${ctx}/img/view.gif"
									alt="Visualizar Resumo da Finalização/Substituição"
									title="Visualizar Resumo da Finalização/Substituição"/>
							</html:link>
	                   </td>
	                   <td>
	                   		<h:commandLink id="ignorar" title="Ignorar Finalização de Bolsista" action="#{homologacaoBolsistaPesquisaBean.ignorarBolsista}" onclick="#{confirmDelete}">
