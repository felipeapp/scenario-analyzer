...
<%@include file="../../include_painel.jsp"%>
	...
	<div class="infoAltRem">
		 <h:graphicImage value="/img/adicionar.gif"style="overflow: visible;"/><h:commandLink action="#{livro.preCadastrar}" value="Cadastrar Livro"/><h:graphicImage value="/img/alterar.gif"style="overflow: visible;"/>: Alterar Livro
	    <h:graphicImage value="/img/delete.gif" style="overflow: visible;"/>: Remover Livro
	...
	<h:outputText value="#{livro.create}" />
	
<c:set var="lista" value="${livro.all}" />
<c:if test="${empty lista}">
	<br />
	<center>
	<span style="color:red;">Nenhum Livro Encontrado.</span>
	</center>
</c:if>
<c:if test="${not empty lista}">
	...
	<table class="listagem" style="width:100%" border="1">
		<caption class="listagem">Lista de Livros</caption>
		<thead>
			<tr>
				<td></td>
				<td>Título</td>
		...
		<h:form>
		<c:forEach items="#{livro.all}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				...
				<td width=20>
					<h:commandLink action="#{livro.atualizar}" >
						<h:graphicImage value="/img/alterar.gif" style="overflow: visible;" title="Alterar"/>
						<f:param name="id" value="#{item.id}"/>
					</h:commandLink>
				</td>						
				<td width=25>
					<h:commandLink action="#{livro.remover}"  onclick="#{confirmDelete}">
...
<h:outputText value="#{livro.dropList}" />
</f:view>
