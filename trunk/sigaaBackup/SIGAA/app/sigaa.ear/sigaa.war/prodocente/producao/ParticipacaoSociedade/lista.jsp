...
<f:view>
	...
	<div class="infoAltRem">
		 <h:graphicImage value="/img/adicionar.gif"style="overflow: visible;"/><h:commandLink action="#{participacaoSociedade.preCadastrar}" value="Cadastrar Participação em Sociedades Científicas e Culturais"/><h:graphicImage value="/img/alterar.gif"style="overflow: visible;"/>: Alterar
	    <h:graphicImage value="/img/delete.gif" style="overflow: visible;"/>: Remover
	</div>
	</h:form>

	<h:outputText value="#{participacaoSociedade.create}" />
	
<c:set var="lista" value="${participacaoSociedade.all}" />
<c:if test="${empty lista}">
	<br />
	<center>
	<span style="color:red;">Nenhuma Participação em Sociedade Científica e Cultural Encontrada.</span>
	</center>
</c:if>
<c:if test="${not empty lista}">
	...
	<table class=listagem style="width:100%" border="1">
		<caption class="listagem">Lista de Participação em Sociedades Científicas e Culturais</caption>
		<thead>
			<td></td>
			<td>Nome</td>
			<td>Tipo Participação</td>
		...
		<h:form>
		<c:forEach items="#{participacaoSociedade.all}" var="item" varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				...
				<td>${item.tipoParticipacaoSociedade.descricao}</td>
				<td style="text-align: center;">${item.anoReferencia}</td>
				<td>${item.situacaoDesc}</td>

				<td width=20>
					<h:commandLink action="#{participacaoSociedade.atualizar}" >
						<h:graphicImage value="/img/alterar.gif" style="overflow: visible;" title="Alterar"/>
						<f:param name="id" value="#{item.id}"/>
					</h:commandLink>
				</td>						
				<td width=25>
					<h:commandLink action="#{participacaoSociedade.remover}" onclick="#{confirmDelete}">
...
<h:outputText value="#{participacaoSociedade.dropList}" />
</f:view>
