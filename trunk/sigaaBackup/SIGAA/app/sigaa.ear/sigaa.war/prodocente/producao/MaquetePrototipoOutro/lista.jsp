...
<f:view>
	...
	<h2>
		...
		Maquetes, Prototipos, Softwares e Outros
	</h2>
	<h:form>
	<div class="infoAltRem">
		 <h:graphicImage value="/img/adicionar.gif"style="overflow: visible;"/><h:commandLink action="#{maquetePrototipoOutro.preCadastrar}" value="Cadastrar Maquetes, Prototipos, Softwares e Outros"/><h:graphicImage value="/img/alterar.gif"style="overflow: visible;"/>: Alterar
	...
	<h:outputText value="#{maquetePrototipoOutro.create}" />
	
<c:set var="lista" value="${maquetePrototipoOutro.all}" />
<c:if test="${empty lista}">
	<br />
	<center>
	<span style="color:red;">Nenhuma Maquetes, Prototipos, Softwares e Outros Encontrada.</span>
	</center>
</c:if>
<c:if test="${not empty lista}">
	...
	<table class=listagem style="width:100%" border="1">
		<caption class="listagem">Lista de Maquetes, Prototipos, Softwares e Outros</caption>
		<thead>
			<tr>
			<td></td>
			<td>Título</td>
			...
			<td>Tipo de Produção Tecnológica</td>
		...
		<h:form>
		<c:forEach items="#{maquetePrototipoOutro.all}" var="item"varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				...
				<td>${item.tipoProducaoTecnologica.descricao}</td>
				<td style="text-align: center;">${item.anoReferencia}</td>
				<td>${item.situacaoDesc}</td>

				<td width=20>
					<h:commandLink action="#{maquetePrototipoOutro.atualizar}" >
						<h:graphicImage value="/img/alterar.gif" style="overflow: visible;" title="Alterar"/>
						<f:param name="id" value="#{item.id}"/>
					</h:commandLink>
				</td>						
				<td width=25>
					<h:commandLink action="#{maquetePrototipoOutro.remover}" onclick="#{confirmDelete}">
...
<h:outputText value="#{maquetePrototipoOutro.dropList}" />
</f:view>
