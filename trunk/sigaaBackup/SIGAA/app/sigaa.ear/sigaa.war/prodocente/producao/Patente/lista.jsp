...
<f:view>
	...
	<div class="infoAltRem">
		 <h:graphicImage value="/img/adicionar.gif"style="overflow: visible;"/><h:commandLink action="#{patente.preCadastrar}" value="Cadastrar Patente"/><h:graphicImage value="/img/alterar.gif"style="overflow: visible;"/>: Alterar
	...
	<h:outputText value="#{patente.create}" />
	<h:outputText value="#{patente.initMBean}" />


<c:set var="lista" value="${patente.all}" />
<c:if test="${empty lista}">
	<br />
	<center>
	<span style="color:red;">Nenhuma Patente Encontrada.</span>
	</center>
</c:if>
<c:if test="${not empty lista}">
	...
	<table class=listagem style="width:100%" border="1">
		<caption class="listagem">Lista de Patentes</caption>
		<thead>
			<tr>
				<td></td>
				<td>Título</td>
				...
				<td style="text-align: left;">Status da patente</td>
				<td>Situação</td>
				<td></td>
				<td></td>
				<td></td>
		...
		<h:form>
		<c:forEach items="#{patente.all}" var="item"varStatus="status">
			<tr class="${status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
				...
				<td style="text-align: left;">
				   <c:if test="${item.status ==1}">Em Registro</c:if>
				   <c:if test="${item.status ==2}">Concedida</c:if>
				</td>
				<td>${item.situacaoDesc}</td>

				<td width=20>
					<h:commandLink action="#{patente.atualizar}" >
						<h:graphicImage value="/img/alterar.gif" style="overflow: visible;" title="Alterar"/>
						<f:param name="id" value="#{item.id}"/>
					</h:commandLink>
				</td>						
				<td width=25>
					<h:commandLink action="#{patente.remover}"  onclick="#{confirmDelete}">
	...
	<h:form id="paginacaoForm">
		...
		<h:outputText value="#{patente.dropList}" />
</f:view>
