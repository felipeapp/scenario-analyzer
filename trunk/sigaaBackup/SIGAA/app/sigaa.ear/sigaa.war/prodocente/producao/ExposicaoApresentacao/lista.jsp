...
<f:view>
	...
	<div class="infoAltRem">
		 <h:graphicImage value="/img/adicionar.gif"style="overflow: visible;"/><h:commandLink action="#{exposicao.preCadastrar}" value="Cadastrar Exposição ou Apresentação"/><h:graphicImage value="/img/alterar.gif"style="overflow: visible;"/>: Alterar
	...
	<h:outputText value="#{exposicao.create}" />
	
<c:set var="lista" value="${exposicao.all}" />
<c:if test="${empty lista}">
	<br />
	<center>
	<span style="color:red;">Nenhuma Exposição ou Apresentação Encontrada.</span>
	</center>
</c:if>
<c:if test="${not empty lista}">
	...
	<table class=listagem style="width:100%" border="1">
		<caption class="listagem">Lista de Exposições ou Apresentações</caption>
		<thead>
			<tr>
				<td></td>
				<td>Título</td>
		...
		<h:form>
		<c:forEach items="#{exposicao.all}" var="item" varStatus="status">
...
<h:outputText value="#{exposicao.dropList}" />
</f:view>
