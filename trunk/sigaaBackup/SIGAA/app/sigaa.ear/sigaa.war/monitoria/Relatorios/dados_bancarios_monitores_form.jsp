...
<f:view>
	...
	<h2><ufrn:subSistema /> > Relatório de Dados Bancários dos Monitores</h2>
		 	...
		 	<c:set var="ASSUMIU_MONITORIA" value="<%= String.valueOf(SituacaoDiscenteMonitoria.ASSUMIU_MONITORIA) %>" scope="application"/>	 		 	
	       	<c:forEach items="${monitores}" var="monitor" varStatus="status">
	               <tr class="${ status.index % 2 == 0 ? "linhaPar" : "linhaImpar" }">
	                    ...
	                    <td> <c:if test="${monitor.banco.codigo != 0}">${monitor.banco.codigo} - ${monitor.banco.denominacao}</c:if></td>
	                    <td> ${monitor.agencia}</td>                    
	                    <td> ${monitor.conta}</td>  
	                    <td> ${monitor.operacao}</td>                   
	                    <td> ${monitor.tipoVinculo.descricao} </td>
						<td> ${(monitor.ativo) ? '<font color=blue>ATIVO</font>':'<font color=red>INATIVO</font>'} </td>
	              </tr>
	          </c:forEach>
		 	</tbody>
		 </table>
