...
<f:view>
	...
	<div class="descricaoOperacao">
		...
		<p> A quantidade máxima de inscrições é a quantidade máxima de participantes que foi cadastrado para a ação selecionada.</p>
	</div>
	
	<h:form id="formGerenciaInscricoes">
	
		<div class="infoAltRem">
			<h:graphicImage value="/img/adicionar.gif" />
			<h:commandLink action="#{inscricaoAtividade.preCadastrar}" value="Criar Inscrição">
				<f:param name="idAtividade" value="#{inscricaoAtividade.atividadeSelecionada.id}" />
				<f:param name="numeroInscritos" value="#{inscricaoAtividade.atividadeSelecionada.cursoEventoExtensao.numeroInscritos}" />
			</h:commandLink>
		...
		<c:choose>
			<c:when test="${not empty inscricoes}">
				...
				<table id="tabelaInscricao" width="100%" class="listagem">
					...
					<caption>
						Inscrições Existentes Para a Ação <br/>
						<h:outputText value="#{inscricaoAtividade.atividadeSelecionada.titulo}" />
					</caption>
						...
						<tr class="${count.index % 2 == 0 ? 'linhaPar' : 'linhaImpar'}">
							...
							<td style="text-align: center;">
								<h:commandLink title="Relatório de Inscritos" action="#{inscricaoAtividade.exibirRelatorioInscritos}" 
											rendered="#{inscricaoAtividade.atividadeSelecionada.cursoEventoExtensao.numeroInscritos > 0}" id="relatorioInscrito">
									<f:param name="idAtividade" value="#{inscricaoAtividade.atividadeSelecionada.id}" />
							...
							<td style="text-align: center;">
								<h:commandLink title="Listar Inscrições" action="#{inscricaoAtividade.listarInscricoesAtividade}"
											 rendered="#{inscricaoAtividade.atividadeSelecionada.cursoEventoExtensao.numeroInscritos > 0}" id="listarInscritos">
									<f:param name="idAtividade" value="#{inscricaoAtividade.atividadeSelecionada.id}" />
							...
							<td style="text-align: center;">
								<h:commandLink title="Alterar Inscrição" action="#{inscricaoAtividade.preAlterar}" id="alterarInscricao" 
										immediate="true">
									<f:param name="id" value="#{inscricao.id}" />
							...
							<td style="text-align: center;">
								<c:choose>
									...
									<c:otherwise>
										<h:commandLink title="Excluir Inscrição" action="#{inscricaoAtividade.excluir}" id="excluirInscricao"
