   ...
   <div class="descricaoOperacao">
	...
	<tbody>  
		...
		<tr>
		 ...
		 <tr>
			<td>
			<h:selectBooleanCheckbox value="#{participanteAcaoExtensao.checkGerarRelatorio}"  id="selectGerarRelatorio" />
			</td>
	...
	<tfoot>
		<tr>
			<td colspan="3">
			<h:commandButton value="Buscar" action="#{ participanteAcaoExtensao.buscarParticipantes }">
			</h:commandButton>
	...
	<br />
		...
		<div class="infoAltRem">
			<c:if test="${acesso.coordenadorExtensao}">
				...
				<h:commandLink action="#{participanteAcaoExtensao.preCadastrar}" style="border: 0;" 
						 value=" Cadastrar Participante">
						<f:param name="id" value="#{participanteAcaoExtensao.acao.id}"/>
				</h:commandLink>
				<h:graphicImage value="/img/extensao/user1_refresh.png" style="overflow: visible;" />: Alterar
				<h:graphicImage value="/img/extensao/user1_delete.png" style="overflow: visible;" />: Remover
			</c:if>
			...
			<c:if test="${acesso.coordenadorExtensao}">
				...
				<br/><h:graphicImage value="/img/email_go.png" style="overflow: visible;"/>: Reenviar senha de acesso
			</c:if> 
		</div>
		
		
		<table class="listagem">
			<caption class="listagem">Lista de Participantes da Ação Selecionada</caption>
			<thead>
				<tr>
			...
			<tbody>
				<tr>
					<td class="subFormulario" colspan="15">${participanteAcaoExtensao.acao.codigoTitulo}</td>
				</tr>
				...
				<c:forEach items="#{participanteAcaoExtensao.resultadosBusca}" var="participante" varStatus="status">
				      ...
				      <tr class="${status.index % 2 == 0 ? 'linhaPar' : 'linhaImpar'}">
						...
						<td width="2%">
							<h:commandLink title="Alterar" action="#{participanteAcaoExtensao.atualizar}" 
									style="border: 0;" id="alterar" rendered="#{acesso.coordenadorExtensao}">
						...
						<td width="2%">
							<h:commandLink title="Remover" action="#{participanteAcaoExtensao.preRemover}" 
									style="border: 0;" id="remover" rendered="#{acesso.coordenadorExtensao}">
								<f:param name="id" value="#{participante.id}" />
								<h:graphicImage url="/img/extensao/user1_delete.png" />
							</h:commandLink>
						</td>
						...
						<td width="2%">
							<h:commandLink title="Emitir declaração" style="border: 0;" id="emitirDeclaracao"
									action="#{declaracaoParticipanteExtensao.emitirDeclaracao}" rendered="#{ !participante.passivelEmissaoCertificado }">
						...
						<td>
				            <h:commandLink title="Reenviar senha de acesso" immediate="true" id="reenviarCodigoSenha"
                                  action="#{participanteAcaoExtensao.reenviarCodigoSenhaAcesso}" 
                                  rendered="#{participanteAcaoExtensao.acao.tipoAtividadeExtensao.id == 3 || participanteAcaoExtensao.acao.tipoAtividadeExtensao.id == 4}">
                                 <f:param name="id" value="#{participante.id}" />
			...
			<tfoot>
				<tr>
					<td colspan="15" align="center">
						<h:commandButton value="Confirmar Alterações" id="btnConfirmarAlteracoes" action="#{participanteAcaoExtensao.atualizarParticipacoes}" />
						<h:panelGroup rendered="#{origem != 'cadastroDeRelatorios'}">
