...
<table>
	...
	<tr>
		<th>Ano-Período:</th>
		<td><h:outputText value="#{portalResultadoAvaliacao.parametroProcessamento.ano}"/>.<h:outputText value="#{portalResultadoAvaliacao.parametroProcessamento.periodo}"/></td>
	</tr>
</table>
</div>
<br/>
...
<c:forEach items="#{portalResultadoAvaliacao.resultadosDocentes}" var="linha">
...
<br/>
<c:if test="${not empty portalResultadoAvaliacao.observacoesDocenteTurma || not empty portalResultadoAvaliacao.observacoesTrancamento}">
	<hr style="color: black; background-color: black;" />
	<h4>Comentários Dados Pelos Discentes na Avaliação Institucional</h4><br/>
</c:if>


<c:if test="${not empty portalResultadoAvaliacao.observacoesDocenteTurma}">
	<c:set var="docenteTurmaAnterior" value="0" />
	<c:set var="index" value="1" />
	<table class="tabelaRelatorioBorda" width="100%">
		<caption>Observações Dadas Pelos Discentes ao Docente da Turma</caption>
		<tbody>
			<c:forEach items="#{portalResultadoAvaliacao.observacoesDocenteTurma}" var="item">
				<c:if test="${docenteTurmaAnterior != item.docenteTurma.id}">
					<tr>
						<td colspan="2" class="caixaCinza"><b>${item.docenteTurma.turma}</b></td>
					</tr>
					<tr>
						<td class="caixaCinza"><b>Discente<acronym title="Os discentes são ordenados aleatoriamente e o índice serve apenas para visualização">*</acronym></b></td>
						<td class="caixaCinza"><b>Observação</b></td>
					</tr>
					<c:set var="docenteTurmaAnterior" value="${item.docenteTurma.id}" />
					<c:set var="index" value="1" />
				</c:if>
				<tr>
					<td style="text-align: right;">${index}</td>
					<td><h:outputText value="#{item.observacoesModeradas}" /></td>
				</tr>
...
<c:if test="${not empty portalResultadoAvaliacao.observacoesTrancamento}">
	<c:set var="turmaAnterior" value="0" />
	<c:set var="index" value="1" />
	<table class="tabelaRelatorioBorda" width="100%">
		<caption>Observações Dadas Pelos Discentes ao Trancamento da Turma</caption>
		<tbody>
			<c:forEach items="#{portalResultadoAvaliacao.observacoesTrancamento}" var="item">
				<c:if test="${turmaAnterior != item.turma.id}">
					<tr>
						<td colspan="2" class="caixaCinza"><b>${item.turma}</b></td>
					</tr>
					<tr>
						<td class="caixaCinza"><b>Discente<acronym title="Os discentes são ordenados aleatoriamente e o índice serve apenas para visualização">*</acronym></b></td>
						<td class="caixaCinza"><b>Observação</b></td>
					</tr>
					<c:set var="turmaAnterior" value="${item.turma.id}" />
					<c:set var="index" value="1" />
				</c:if>
				<tr>
					<td style="text-align: right;">${index}</td>
...
<c:forEach items="#{portalResultadoAvaliacao.detalheRespostas}" var="turma">
		...
		<c:forEach items="${turma.value}" var="grupoPergunta">
			...
			<table class="tabelaRelatorioBorda" width="100%">
				<thead>
					...
					<tr>
						<th width="15%" rowspan="2" style="text-align: right;">Discente<acronym title="Os discentes são ordenados aleatoriamente e o índice serve apenas para visualização">*</acronym></th>
				...
				<tbody>
					...
					<%-- TabelaRespostaResultadoAvaliacao --%>
					<tr>
						<td align="right">${discenteStatus.index + 1}</td>
						<c:forEach items="${discentes.value}" var="respostas" varStatus="respostaStatus">
							<c:if test="${respostas.pergunta.simNao}">
								<td  align="center" style="${respostas.valorSimNao eq "N" ? "color: red" : " "}">
...
<table>
	...
	<c:forEach items="#{portalResultadoAvaliacao.detalheRespostas}" var="turma" end="0">
		<c:forEach items="${turma.value}" var="grupoPergunta">
			<tr><td colspan="2"><b>${grupoPergunta.key.titulo}</b> - ${grupoPergunta.key.descricao }</td></tr>
			<c:forEach items="${grupoPergunta.value.mapaRespostas}" var="discentes" varStatus="discenteStatus" end="0">
				<c:forEach items="${discentes.value}" var="respostas" varStatus="respostaStatus">
					<tr>
						<td valign="top"><b>${grupoPergunta.key.id}.${respostaStatus.index + 1}</b></td>
