		...
		<div class="notas" style="clear: both;">
			...
			<thead>
				<tr>
					...
					<c:forEach var="unid" items="${ bean.notas }">
						...
						<c:if test="${ bean.ead }">
						<th style="text-align: center">Prof. ${ unid.unidade } x ${ bean.pesoProfessor }%</th>
						<th style="text-align: center">Tutor ${ unid.unidade }</th>
						<th style="text-align: center">Tutor ${ unid.unidade } x ${ bean.pesoTutor }%</th>
						</c:if>
					</c:forEach>
			...
			<c:forEach var="matricula" items="${ bean.turma.matriculasDisciplina }" varStatus="i">
				...
				<c:forEach var="unid" items="${ matricula.notas }" varStatus="loop">
					...
					<td style="border-right: 1px solid #888; text-align: center"><label><fmt:formatNumber pattern="#0.0" value="${ matricula.notaTutor }"/></label></td>
					<td style="border-right: 1px solid #888; text-align: center"><fmt:formatNumber pattern="#0.0" value="${ empty matricula.notaTutor ? '' : matricula.notaTutor * bean.pesoTutor / 100.0 }"/></td>
					</c:if>
					<c:if test="${ unid.unidade == 2 }">
					<td  style="border-right: 1px solid #888; text-align: center"><label><fmt:formatNumber pattern="#0.0" value="${ matricula.notaTutor2 }"/></label></td>
					<td style="border-right: 1px solid #888; text-align: center"><fmt:formatNumber pattern="#0.0" value="${ empty matricula.notaTutor2 ? '' : matricula.notaTutor2 * bean.pesoTutor / 100.0 }"/></td>
					</c:if>
					</c:if>
