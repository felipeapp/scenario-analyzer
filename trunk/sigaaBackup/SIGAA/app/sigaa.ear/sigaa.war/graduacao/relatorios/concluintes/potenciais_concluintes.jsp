...
<f:view>
    <h2 class="tituloTabela"><b>${relatoriosConcluintesMBean.tituloRelatorio}</b></h2>
    <div id="parametrosRelatorio">
		<table>
			<tr>
				<th>Unidade:</th>
				<c:if test="${empty relatoriosConcluintesMBean.unidade.nome}">
					<td>TODOS</td>			
				</c:if>
				<c:if test="${!empty relatoriosConcluintesMBean.unidade.nome}">
					<td><h:outputText value="#{relatoriosConcluintesMBean.unidade.nome}"/></td>
				</c:if>	
			</tr>	   
			<tr>
				<th>Ano: </th>
				<td>
				    ${relatoriosConcluintesMBean.ano}
				</td>		
			</tr>	
			<c:if test="${relatoriosConcluintesMBean.potenciaisConcluintes}">
				<tr>
					<th>Semestre: </th>
					<td>
					    ${relatoriosConcluintesMBean.periodo}
					</td>		
				</tr>			
			</c:if>	
		</table>
	...
	<table class="tabelaRelatorioBorda" width="100%">
	    ...
	    <tbody>
		 	<c:forEach items="#{relatoriosConcluintesMBean.listagem}"  var="linha" varStatus="loop">
	 			...
	 			<c:if test="${idCurso != linha.id_curso}">
					...
					<tr>
						<th class="modalidade" colspan="6" style="text-align: center;"><b>${linha.curso} ( ${linha.me_modalidade} )</b></th>
					</tr>	
					<c:set var="imprimiuTotal" value="false"/>		
					<c:set var="idCurso" value="${linha.id_curso}"/>
					<c:set var="nomeCurso" value="${linha.curso}"/> 	 		    
	 			</c:if>			 	
