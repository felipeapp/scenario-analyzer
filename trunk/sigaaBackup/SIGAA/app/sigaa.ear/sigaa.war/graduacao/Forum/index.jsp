...
<table style="margin: 20px auto; border: 1px solid #C4D2EB; width: 99%" cellpadding="3">
	...
	<c:forEach items="${forum.allTurma}" var="item">
		<tr class="${ loop.index % 2 == 0 ? 'linhaPar' : 'linhaImpar' }">
			...
			<td>
				...
				<h:commandButton image="/img/alterar.gif" value="Alterar" action="#{forum.atualizar}" />
				</h:form>
			</td>
			<td>
				<h:form>
				<input type="hidden" value="${item.id}" name="id" /> 
				<h:commandButton image="/img/delete.gif" alt="Remover" action="#{forum.remover}" onclick="return(confirm('Deseja realmente excluir este registro?'))"/>
				</h:form>
			</td>
			</c:if>
		</tr>
...
<h:form>
	...
	<ul>
		<li>
			...
			<h:inputText id="titulo" value="#{forum.obj.titulo}" readonly="#{forum.readOnly}" style="width: 95%"/>
		</li>
		<li>
			...
			<t:inputTextarea id="descricao" rows="2" value="#{forum.obj.descricao}" style="width: 95%"/>
		</li>
		<li>
			...
			<t:selectOneRadio id="tipo" value="#{forum.obj.topicos}" styleClass="noborder">
		...
		<li>
			...
			<t:selectOneRadio id="email" value="#{forum.notificar}" styleClass="noborder">
				<f:selectItem itemLabel="Sim" itemValue="true"/>
				<f:selectItem itemLabel="Não" itemValue="false"/>
			</t:selectOneRadio>
		</li>
		<li class="botoes">
			<h:inputHidden value="#{forum.confirmButton}" />
			<h:inputHidden value="#{forum.obj.id}" />
			<h:commandButton value="#{forum.confirmButton}" action="#{forum.cadastrar}" /> 
			<h:commandButton value="Cancelar" action="#{forum.cancelar}" />
		</li>
	</ul>
	</fieldset>
</h:form>
