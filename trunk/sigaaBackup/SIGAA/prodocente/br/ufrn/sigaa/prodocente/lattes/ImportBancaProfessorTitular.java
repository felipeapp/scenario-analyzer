...
/**
 * Classe que extrai dados referentes a Participação em Banca de Concurso de Professor titular
 * 
 * @author David Pereira
 *
 */
public class ImportBancaProfessorTitular extends ImportProducao<Banca> {

	public ImportBancaProfessorTitular(InputStream input, ImportLattesDao dao, List<? extends Producao> producoes) {
		super(input, dao, producoes, "BANCA-JULGADORA-PARA-PROFESSOR-TITULAR");
	}

	@Override
	public void startElement(String uri, String localName, String qName, Attributes attributes) throws SAXException {

		if ("BANCA-JULGADORA-PARA-PROFESSOR-TITULAR".equals(qName)) {
			item = new Banca();
			item.setTipoBanca(new TipoBanca(TipoBanca.CONCURSO));
			item.setCategoriaFuncional(CategoriaFuncional.PROF_TITULAR);
			item.setNaturezaExame(null);
			item.setSequenciaProducao(parseInt(attributes.getValue("SEQUENCIA-PRODUCAO")));
			
			item.setTipoProducao(TipoProducao.BANCA_CONCURSO);
		...
		if ("DADOS-BASICOS-DA-BANCA-JULGADORA-PARA-PROFESSOR-TITULAR".equals(qName) && parse) {
		...
		if ("DETALHAMENTO-DA-BANCA-JULGADORA-PARA-PROFESSOR-TITULAR".equals(qName) && parse) {
