...
public abstract class AnalyzerReportUtil {
	...
	public static void saveAVGs(String message, String filename, Map<String, Double> avgs, Collection<String> elements)
	...
	public static void saveElements(String message, String filename, Collection<StatElement> collection, double rate,
			double alpha) throws FileNotFoundException {
		...
		for (StatElement e : collection) {
			pw.println(e.getElementName() + ";" + e.getTTestPvalue() + ";" + e.getUTestPvalue() + ";" + e.getAVGv1()
					+ ";" + e.getAVGv2() + ";" + e.getN1() + ";" + e.getN2());
		}
	...
	public static void saveFails(String message, String filename,
			Map<RuntimeScenario, List<RuntimeNode>> scenario_to_nodes, Map<String, Integer> failed_methods, int release)
			throws FileNotFoundException {
	...
	public static void saveCollection(String message, String filename, Collection<String> collection, boolean hasHeader)
			throws FileNotFoundException {
		System.out.println("Saving >> " + message);
	...
	public static void saveFullMiningData(String message, String filename,
			Map<String, Collection<UpdatedMethod>> path_to_upmethod, Collection<Long> issue_numbers,
			Map<String, Integer> issuetype_to_count, Map<String, Collection<UpdatedMethod>> issuetype_to_upmethod)
			throws FileNotFoundException {
		...
		for (String path : path_to_upmethod.keySet()) {
			...
			for (UpdatedMethod upmethod : upmethods_collection) {
				...
				pw.println("\t" + upmethod.getMethodLimit().getSignature() + ";"
						+ upmethod.getMethodLimit().getStartLine() + ";" + upmethod.getMethodLimit().getEndLine());
	...
	public static void saveImpactedElements(String message, String filename, Collection<String> methods)
			throws FileNotFoundException {
		System.out.println("Saving >> " + message);
		...
		for (String signature : methods) {
			System.out.println(
					"[" + ++i + "/" + methods.size() + "] Retrieving impacted members and scenarios by " + signature);
	...
	public static void saveCodeAssets(String message, String filename, Map<String, Integer> total_classes,
			Map<String, Integer> total_packages) throws FileNotFoundException {
		System.out.println("Saving >> " + message);
	...
	public static String loadCollection(Collection<String> collection, String filename, boolean header)
			throws IOException {
		...
		for (int i = 0; i < number_of_registers; i++)
			collection.add(br.readLine().split(";")[0]); // I just need the
															// first token:
															// element name

		/*
		 * This method should read the performance rate and the significance
		 * level here. However I do not need them at this moment. So, I will
	...
	public static String loadTimeOfChangedElements(Map<String, Double> signature_to_avg, String filename)
			throws IOException {
		BufferedReader br = new BufferedReader(new FileReader(filename));
	...
	public static String loadTimeOfKeptElements(Map<String, Double> signature_to_avgr1,
			Map<String, Double> signature_to_avgr2, String filename) throws IOException {
		BufferedReader br = new BufferedReader(new FileReader(filename));
		...
		/*
		 * Just read the header = Name;P-Value (TTest);P-Value (UTest);Mean
		 * R1;Mean R2;N1;N2 0 1 2 3 4 5 6
		 */
		br.readLine();
	...
	public static String getSelectedCommitHeaderForRAnalysis(String separator) {
		...
		sb.append("authorexpertise" + separator); // Number of author's commits
													// before the commit
		sb.append("daysbeforerelease" + separator); // Days from the commit date
													// to the release date
	...
	public static void saveCommitsForRAnalysis(Set<Commit> all_commits, Set<String> blamed_commits, String filename,
			String separator) throws FileNotFoundException {
		PrintWriter pw = new PrintWriter(filename);
		Set<String> lines = new TreeSet<String>(); // A TreeSet, just to keep
													// the order
		...
		for (Commit commit : all_commits)
			lines.add(blamed_commits.contains(commit.getRevision()) + separator
					+ AnalyzerCollectionUtil.getCommitSelectedProperties(commit, separator));
	...
	/*
	 * # List of all commits from changed methods found during the repository
	 * mining phase (blamed or not) 264 Revision,Bug Fixing,Author,Date,Number
	 * of Packages,Number of Files,Number of Insertions,Number of
	 * Deletions,Number of Churns,Delta of Lines,Number of Hunks,Number of
	 * Issues,Issues 17727,false,tfmorris,Sáb 02-jan-2010 22:23:40
	 * UTC,4,6,272,7,279,265,13,1,0
	 */
	public static List<Map<String, String>> loadCommitReport(String filename, String separator) throws IOException {
		...
		String[] header_tokens = br.readLine().split(separator); // Header of
																	// the file
